...
// The V61 release root attests the current catalog and ACL state. Provider data
...
async function verifyCompatibilityAndBoundary(): Promise<void> {
  // V61 is the only live oracle. Historical projections are observed only to
  ...
  >`
    SELECT
      has_function_privilege(
        'periapsis_api', 'app.schema_compatibility_v61()'::regprocedure,
        'EXECUTE'
      ) AS "apiV61",
      has_function_privilege(
        'periapsis_worker', 'app.schema_compatibility_v61()'::regprocedure,
        'EXECUTE'
      ) AS "workerV61",
      has_function_privilege(
