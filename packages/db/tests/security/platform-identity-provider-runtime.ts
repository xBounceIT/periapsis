...
// The V62 release root attests the current catalog and ACL state. Provider data
...
async function verifyCompatibilityAndBoundary(): Promise<void> {
  // V62 is the only live oracle. Historical projections are observed only to
  ...
  >`
    SELECT
      has_function_privilege(
        'periapsis_api', 'app.schema_compatibility_v62()'::regprocedure,
        'EXECUTE'
      ) AS "apiV62",
      has_function_privilege(
        'periapsis_worker', 'app.schema_compatibility_v62()'::regprocedure,
        'EXECUTE'
      ) AS "workerV62",
      has_function_privilege(
      ...
      has_function_privilege(
        'periapsis_api',
        'app.release_runtime_schema_readiness_v62()'::regprocedure,
        'EXECUTE'
      ) AS "apiReadiness",
      has_function_privilege(
        'periapsis_worker',
