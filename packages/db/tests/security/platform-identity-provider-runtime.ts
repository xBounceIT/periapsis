...
// The V58 release root attests the current catalog and ACL state. Provider data
...
async function verifyCompatibilityAndBoundary(): Promise<void> {
  // V58 is the only live oracle. Historical projections are observed only to
  ...
  >`
    SELECT
      has_function_privilege(
        'periapsis_api', 'app.schema_compatibility_v58()'::regprocedure,
        'EXECUTE'
      ) AS "apiV58",
      has_function_privilege(
        'periapsis_worker', 'app.schema_compatibility_v58()'::regprocedure,
        'EXECUTE'
      ) AS "workerV58",
      has_function_privilege(
