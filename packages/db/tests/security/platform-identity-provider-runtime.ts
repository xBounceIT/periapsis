...
// The V56 release root attests the current catalog and ACL state. Provider data
...
async function verifyCompatibilityAndBoundary(): Promise<void> {
  // V56 is the only live oracle. Historical projections are observed only to
  ...
  >`
    SELECT
      has_function_privilege(
        'periapsis_api', 'app.schema_compatibility_v56()'::regprocedure,
        'EXECUTE'
      ) AS "apiV56",
      has_function_privilege(
        'periapsis_worker', 'app.schema_compatibility_v56()'::regprocedure,
        'EXECUTE'
      ) AS "workerV56",
      has_function_privilege(
