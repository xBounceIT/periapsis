...
// The V51 release root attests the current catalog and ACL state. Provider data
...
async function verifyCompatibilityAndBoundary(): Promise<void> {
  // V51 is the only live oracle. Historical projections are observed only to
  ...
  >`
    SELECT
      has_function_privilege(
        'periapsis_api', 'app.schema_compatibility_v51()'::regprocedure,
        'EXECUTE'
      ) AS "apiV51",
      has_function_privilege(
        'periapsis_worker', 'app.schema_compatibility_v51()'::regprocedure,
        'EXECUTE'
      ) AS "workerV51",
      has_function_privilege(
