...
// The V57 release root attests the current catalog and ACL state. Provider data
...
async function verifyCompatibilityAndBoundary(): Promise<void> {
  // V57 is the only live oracle. Historical projections are observed only to
  ...
  >`
    SELECT
      has_function_privilege(
        'periapsis_api', 'app.schema_compatibility_v57()'::regprocedure,
        'EXECUTE'
      ) AS "apiV57",
      has_function_privilege(
        'periapsis_worker', 'app.schema_compatibility_v57()'::regprocedure,
        'EXECUTE'
      ) AS "workerV57",
      has_function_privilege(
