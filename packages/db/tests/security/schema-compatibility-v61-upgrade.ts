...
assert.deepEqual(
  ...
  ["0252_ticket_comment_authority", "0253_v61_compatibility"],
);
...
assert(v61CatalogDigest, "0253 must pin the V61 catalog digest");
assert.notEqual(
  v61CatalogDigest,
  "0".repeat(64),
...
try {
  ...
  try {
    ...
    throw new Error(
      "ordinary SAML admission/runtime proof failed after V60 -> V61 upgrade",
    );
  }
  await assertSealedV61();
  assert.deepEqual(
