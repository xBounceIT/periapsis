...
assert.deepEqual(
  journal.entries.slice(-16).map((entry) => entry.tag),
  [
...
assert(v60CatalogDigest, "0239 must pin the V60 catalog digest");
assert.notEqual(
  v60CatalogDigest,
  "0".repeat(64),
...
try {
  ...
  try {
    ...
    throw new Error(
      "ordinary SAML admission/runtime proof failed after V52 -> V60 upgrade",
    );
  }
  await assertSealedV60();
  assert.deepEqual(
