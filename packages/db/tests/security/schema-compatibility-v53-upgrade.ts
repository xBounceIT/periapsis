...
assert.deepEqual(
  journal.entries.slice(-12).map((entry) => entry.tag),
  [
...
assert(v58CatalogDigest, "0239 must pin the V58 catalog digest");
assert.notEqual(
  v58CatalogDigest,
  "0".repeat(64),
...
try {
  ...
  try {
    ...
    throw new Error(
      "ordinary SAML admission/runtime proof failed after V52 -> V58 upgrade",
    );
  }
  await assertSealedV58();
  assert.deepEqual(
