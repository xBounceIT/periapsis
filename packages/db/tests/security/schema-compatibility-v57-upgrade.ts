...
assert.deepEqual(
  ...
  ["0244_sla_authority_epochs", "0245_v57_compatibility"],
);
...
assert(v57CatalogDigest, "0245 must pin the V57 catalog digest");
assert.notEqual(
  v57CatalogDigest,
  "0".repeat(64),
...
try {
  ...
  try {
    ...
    throw new Error(
      "ordinary SAML admission/runtime proof failed after V56 -> V57 upgrade",
    );
  }
  await assertSealedV57();
  assert.deepEqual(
