...
const retiredV60Roots = [
  "schema_compatibility_v60",
  "release_runtime_schema_readiness_v60",
  "federated_authentication_schema_readiness_v60",
  "platform_oidc_direct_runtime_schema_readiness_v60",
  "platform_saml_direct_runtime_schema_readiness_v60",
  "platform_local_account_runtime_schema_readiness_v60",
  "sla_trigger_action_runtime_schema_readiness_v60",
  "sla_object_event_ingress_schema_readiness_v60",
  "ticket_bulk_runtime_schema_readiness_v60",
  "ticket_export_runtime_schema_readiness_v60",
  "ticket_metadata_runtime_schema_readiness_v60",
  "notification_dispatch_readiness_v60",
  "api_runtime_schema_readiness_v60",
  "worker_runtime_schema_readiness_v60",
] as const;

const retainedPrivateV60Roots = [
  "private_schema_compatibility_journal_v60",
  "private_release_runtime_dependency_surface_hash_v60",
  "private_release_runtime_schema_readiness_v60",
] as const;

const retiredV61Roots = [
  "schema_compatibility_v61",
  "release_runtime_schema_readiness_v61",
  "federated_authentication_schema_readiness_v61",
  "platform_oidc_direct_runtime_schema_readiness_v61",
  "platform_saml_direct_runtime_schema_readiness_v61",
  "platform_local_account_runtime_schema_readiness_v61",
  "sla_trigger_action_runtime_schema_readiness_v61",
  "sla_object_event_ingress_schema_readiness_v61",
  "ticket_bulk_runtime_schema_readiness_v61",
  "ticket_export_runtime_schema_readiness_v61",
  "ticket_metadata_runtime_schema_readiness_v61",
  "notification_dispatch_readiness_v61",
  "api_runtime_schema_readiness_v61",
  "worker_runtime_schema_readiness_v61",
] as const;

const retainedPrivateV61Roots = [
  "private_schema_compatibility_journal_v61",
  "private_release_runtime_dependency_surface_hash_v61",
  "private_release_runtime_schema_readiness_v61",
] as const;
...
const v62Migration = await readFile(
  resolve(migrationsRoot, "0255_v62_compatibility.sql"),
  "utf8",
);
const expectedCatalogDigest =
  /private_release_runtime_dependency_surface_hash_v62\(\)<>\s*'([0-9a-f]{64})'/u.exec(
    v62Migration,
  )?.[1];
assert(
  expectedCatalogDigest,
  "0255 must pin the exact V62 release catalog digest",
);
assert.notEqual(
  expectedCatalogDigest,
  "0".repeat(64),
  "0255 must contain the independently derived V62 digest, not a placeholder",
);
assert.equal(
  expectedCatalogDigest,
  "0d25429b2819338277f57b9b5415d2216091dd14c0c0e44791f2c848d98f7ddf",
);
assert.equal(expectedMigrationCount, 256);
assert.equal(expectedMigrationCreatedAt, 1_788_887_610_348);
...
async function readV62Readiness(
  sql: Sql | postgres.TransactionSql,
): Promise<ReadinessRow> {
  const [row] = await sql<ReadinessRow[]>`
    SELECT app.private_release_runtime_dependency_surface_hash_v62()
             AS catalog_digest,
           ARRAY[
             ...
             app.platform_oidc_direct_runtime_schema_readiness_v62(),
             app.platform_saml_direct_runtime_schema_readiness_v62(),
             app.platform_local_account_runtime_schema_readiness_v62(),
             ...
             app.ticket_metadata_runtime_schema_readiness_v62(),
             app.alert_dfir_runtime_schema_readiness_v2(),
             (SELECT readiness.schema_safe
              FROM app.notification_dispatch_readiness_v62() AS readiness)
           ]::boolean[] || app.api_runtime_schema_readiness_v62()
             || app.worker_runtime_schema_readiness_v62() AS readiness
  `;
  assert(row, "V62 readiness returned no row");
...
async function readCatalogDigest(
  ...
  const [row] = await sql<{ digest: string }[]>`
    SELECT app.private_release_runtime_dependency_surface_hash_v62()
             AS digest
  `;
  assert(row, "V62 catalog digest returned no row");
  return row.digest;
}
...
async function readNotifierSourceAttestation(
  ...
  `;
  assert(row, "notifier V62 source attestation returned no row");
  return row.source_safe;
}

const databaseUrl =
  process.env.PERIAPSIS_SCHEMA_COMPATIBILITY_V62_SECURITY_TEST_DATABASE_URL;
assert(
  databaseUrl !== undefined && databaseUrl.trim() !== "",
  "PERIAPSIS_SCHEMA_COMPATIBILITY_V62_SECURITY_TEST_DATABASE_URL is required",
);
...
try {
  ...
  const [compatibility] = await sql<CompatibilityRow[]>`
    SELECT * FROM app.schema_compatibility_v62()
  `;
  assert(compatibility, "schema_compatibility_v62 returned no row");
  assert.deepEqual(
    {
  ...
  const readiness = await readV62Readiness(sql);
  assert.equal(readiness.catalog_digest, expectedCatalogDigest);
  assert.equal(readiness.readiness.length, 26);
  assert(!readiness.readiness.includes(false), "a V62 readiness root failed");
  ...
  const retiredV60Catalog = await readFunctionCatalog(sql, retiredV60Roots);
  assert.equal(retiredV60Catalog.length, 14);
  for (const root of retiredV60Catalog) {
    const notificationRoot =
      root.function_name === "notification_dispatch_readiness_v60";
    assert.equal(
      root.owner_name,
      notificationRoot
        ? "periapsis_notification_readiness_owner"
  ...
  const [retiredV60Privileges] = await sql<{ granted_count: number }[]>`
    SELECT count(*)::integer AS granted_count
    FROM pg_catalog.pg_proc AS function_row
    JOIN pg_catalog.pg_namespace AS namespace
      ON namespace.oid=function_row.pronamespace
    CROSS JOIN unnest(${[...runtimeRoles]}::text[]) AS runtime_role(name)
    WHERE namespace.nspname='app' AND function_row.pronargs=0
      AND function_row.proname=ANY(${[...retiredV60Roots]}::text[])
      AND pg_catalog.has_function_privilege(
        runtime_role.name,function_row.oid,'EXECUTE'
      )
  `;
  assert.deepEqual(retiredV60Privileges, { granted_count: 0 });

  const [retiredV60Compatibility] = await sql<CompatibilityRow[]>`
    SELECT * FROM app.schema_compatibility_v60()
  `;
  assert(retiredV60Compatibility, "retired V60 compatibility returned no row");
  assert.deepEqual(
    {
      ...retiredV60Compatibility,
      applied_count: Number(retiredV60Compatibility.applied_count),
      latest_created_at: Number(retiredV60Compatibility.latest_created_at),
    },
    {
      applied_count: 0,
      latest_created_at: 0,
  ...
  const [retiredV60Config] = await sql<{ config: string[] }[]>`
    SELECT function_row.proconfig AS config
    FROM pg_catalog.pg_proc AS function_row
    WHERE function_row.oid='app.schema_compatibility_v60()'::regprocedure
  `;
  assert.deepEqual(retiredV60Config, {
    config: [
      "search_path=pg_catalog",
      "app.schema_compatibility_fingerprint=RETIRED",
    ],
  ...
  const retiredV61Catalog = await readFunctionCatalog(sql, retiredV61Roots);
  assert.equal(retiredV61Catalog.length, 14);
  for (const root of retiredV61Catalog) {
    const notificationRoot =
      root.function_name === "notification_dispatch_readiness_v61";
    assert.equal(
      root.owner_name,
      notificationRoot
        ? "periapsis_notification_readiness_owner"
  ...
  const [retiredV61Privileges] = await sql<{ granted_count: number }[]>`
    SELECT count(*)::integer AS granted_count
    FROM pg_catalog.pg_proc AS function_row
    JOIN pg_catalog.pg_namespace AS namespace
      ON namespace.oid=function_row.pronamespace
    CROSS JOIN unnest(${[...runtimeRoles]}::text[]) AS runtime_role(name)
    WHERE namespace.nspname='app' AND function_row.pronargs=0
      AND function_row.proname=ANY(${[...retiredV61Roots]}::text[])
      AND pg_catalog.has_function_privilege(
        runtime_role.name,function_row.oid,'EXECUTE'
      )
  `;
  assert.deepEqual(retiredV61Privileges, { granted_count: 0 });

  const [retiredV61Compatibility] = await sql<CompatibilityRow[]>`
    SELECT * FROM app.schema_compatibility_v61()
  `;
  assert(retiredV61Compatibility, "retired V61 compatibility returned no row");
  assert.deepEqual(
    {
      ...retiredV61Compatibility,
      applied_count: Number(retiredV61Compatibility.applied_count),
      latest_created_at: Number(retiredV61Compatibility.latest_created_at),
    },
    {
      applied_count: 0,
      latest_created_at: 0,
  ...
  const [retiredV61Config] = await sql<{ config: string[] }[]>`
    SELECT function_row.proconfig AS config
    FROM pg_catalog.pg_proc AS function_row
    WHERE function_row.oid='app.schema_compatibility_v61()'::regprocedure
  `;
  assert.deepEqual(retiredV61Config, {
    config: [
      "search_path=pg_catalog",
      "app.schema_compatibility_fingerprint=RETIRED",
    ],
  ...
  const notifierCatalog = await readFunctionCatalog(sql, [
    ...
    "notification_dispatch_readiness_v49",
    "notification_dispatch_readiness_v50",
    "notification_dispatch_readiness_v62",
  ]);
  assert.deepEqual(notifierCatalog, [
    ...
    },
    {
      ...
      function_name: "notification_dispatch_readiness_v62",
      owner_name: "periapsis_notification_readiness_owner",
  ...
  await assert.rejects(
    sql.begin(async (transaction) => {
      await transaction`
        ...
        TO periapsis_api
      `;
      const drifted = await readV62Readiness(transaction);
      assert.equal(
      ...
      assert.equal(
        drifted.readiness[1],
        false,
        "the public V62 root must independently reject a restored V50 grant",
      );
  ...
  await assert.rejects(
    sql.begin(async (transaction) => {
      ...
      assert.notEqual(
        drifted.catalog_digest,
        expectedCatalogDigest,
        "the historical V50 private root is no longer self-excluded in V62",
      );
      assert.equal(drifted.readiness[0], false);
      assert.equal(drifted.readiness[1], false);
  ...
  await assert.rejects(
    sql.begin(async (transaction) => {
      ...
      assert.equal(
        drifted.readiness[1],
        false,
        "the public V62 root must independently reject a restored V51 grant",
      );
  ...
  await assert.rejects(
    sql.begin(async (transaction) => {
      ...
      assert.notEqual(
        drifted.catalog_digest,
        expectedCatalogDigest,
        "the historical V51 private root is no longer self-excluded in V62",
      );
      assert.equal(drifted.readiness[0], false);
      assert.equal(drifted.readiness[1], false);
  ...
  await assert.rejects(
    sql.begin(async (transaction) => {
      ...
      assert.equal(
        drifted.readiness[1],
        false,
        "the public V62 root must independently reject a restored V52 grant",
      );
  ...
  await assert.rejects(
    sql.begin(async (transaction) => {
      ...
      assert.notEqual(
        drifted.catalog_digest,
        expectedCatalogDigest,
        "the historical V52 private root is no longer self-excluded in V62",
      );
      assert.equal(drifted.readiness[0], false);
      assert.equal(drifted.readiness[1], false);
  ...
  await assert.rejects(
    sql.begin(async (transaction) => {
      ...
      assert.equal(
        drifted.readiness[1],
        false,
        "the public V62 root must independently reject a restored V53 grant",
      );
  ...
  await assert.rejects(
    sql.begin(async (transaction) => {
      ...
      assert.notEqual(
        drifted.catalog_digest,
        expectedCatalogDigest,
        "the historical V53 private root is no longer self-excluded in V62",
      );
      assert.equal(drifted.readiness[0], false);
      assert.equal(drifted.readiness[1], false);
  ...
  await assert.rejects(
    sql.begin(async (transaction) => {
      ...
      assert.equal(
        drifted.readiness[1],
        false,
        "the public V62 root must independently reject a restored V54 grant",
      );
  ...
  await assert.rejects(
    sql.begin(async (transaction) => {
      ...
      assert.notEqual(
        drifted.catalog_digest,
        expectedCatalogDigest,
        "the historical V54 private root is no longer self-excluded in V62",
      );
      assert.equal(drifted.readiness[0], false);
      assert.equal(drifted.readiness[1], false);
  ...
  await assert.rejects(
    sql.begin(async (transaction) => {
      ...
      assert.equal(
        drifted.readiness[1],
        false,
        "the public V62 root must independently reject a restored V55 grant",
      );
  ...
  await assert.rejects(
    sql.begin(async (transaction) => {
      ...
      assert.notEqual(
        drifted.catalog_digest,
        expectedCatalogDigest,
        "the historical V55 private root is no longer self-excluded in V62",
      );
      assert.equal(drifted.readiness[0], false);
      assert.equal(drifted.readiness[1], false);
  ...
  await assert.rejects(
    sql.begin(async (transaction) => {
      ...
      assert.equal(
        drifted.readiness[1],
        false,
        "the public V62 root must independently reject a restored V56 grant",
      );
  ...
  await assert.rejects(
    sql.begin(async (transaction) => {
      ...
      assert.notEqual(
        drifted.catalog_digest,
        expectedCatalogDigest,
        "the historical V56 private root is no longer self-excluded in V62",
      );
      assert.equal(drifted.readiness[0], false);
      assert.equal(drifted.readiness[1], false);
  ...
  await assert.rejects(
    sql.begin(async (transaction) => {
      ...
      assert.equal(
        drifted.readiness[1],
        false,
        "the public V62 root must independently reject a restored V57 grant",
      );
  ...
  await assert.rejects(
    sql.begin(async (transaction) => {
      ...
      assert.notEqual(
        drifted.catalog_digest,
        expectedCatalogDigest,
        "the historical V57 private root is no longer self-excluded in V62",
      );
      assert.equal(drifted.readiness[0], false);
      assert.equal(drifted.readiness[1], false);
  ...
  await assert.rejects(
    sql.begin(async (transaction) => {
      ...
      assert.equal(
        drifted.readiness[1],
        false,
        "the public V62 root must independently reject a restored V58 grant",
      );
  ...
  await assert.rejects(
    sql.begin(async (transaction) => {
      ...
      assert.notEqual(
        drifted.catalog_digest,
        expectedCatalogDigest,
        "the historical V58 private root is no longer self-excluded in V62",
      );
      assert.equal(drifted.readiness[0], false);
      assert.equal(drifted.readiness[1], false);
  ...
  await assert.rejects(
    sql.begin(async (transaction) => {
      ...
      assert.equal(
        drifted.readiness[1],
        false,
        "the public V62 root must independently reject a restored V59 grant",
      );
  ...
  await assert.rejects(
    sql.begin(async (transaction) => {
      ...
      assert.notEqual(
        drifted.catalog_digest,
        expectedCatalogDigest,
        "the historical V59 private root is no longer self-excluded in V62",
      );
      assert.equal(drifted.readiness[0], false);
      assert.equal(drifted.readiness[1], false);
  ...
  const privateV60Catalog = await readFunctionCatalog(
    sql,
    retainedPrivateV60Roots,
  );
  assert.equal(privateV60Catalog.length, retainedPrivateV60Roots.length);
  for (const root of privateV60Catalog) {
    assert.equal(root.owner_name, "periapsis_migrator");
    assert.deepEqual(root.acl_roles, ["periapsis_migrator"]);
    assert.equal(root.runtime_execute_count, 0);
  }

  const [retiredV60Source] = await sql<{ source_hash: string }[]>`
    SELECT encode(sha256(convert_to(prosrc, 'UTF8')), 'hex') AS source_hash
    FROM pg_catalog.pg_proc WHERE oid='app.schema_compatibility_v60()'::regprocedure
  `;
  assert.equal(
    retiredV60Source?.source_hash,
    expectedRetiredSchemaCompatibilityV60SourceHash,
  );

  const normalizedV60GrantRollback = {
    kind: "normalized-v60-grant",
  } as const;
  await assert.rejects(
    sql.begin(async (transaction) => {
      await transaction`
      ...
      assert.equal(
        drifted.catalog_digest,
        expectedCatalogDigest,
        "the exact V60 ACL rotation is intentionally digest-normalized",
      );
      assert.equal(drifted.readiness[0], true);
      assert.equal(
        drifted.readiness[1],
        false,
        "the public V62 root must independently reject a restored V60 grant",
      );
      throw normalizedV60GrantRollback;
    }),
    (error: unknown) => error === normalizedV60GrantRollback,
  );

  const normalizedV60NotifierGrantRollback = {
    kind: "normalized-v60-notifier-grant",
  } as const;
  await assert.rejects(
    sql.begin(async (transaction) => {
      await transaction`
        GRANT EXECUTE ON FUNCTION app.notification_dispatch_readiness_v60()
        TO periapsis_notifier
      `;
      const drifted = await readV62Readiness(transaction);
      assert.equal(drifted.catalog_digest, expectedCatalogDigest);
      ...
      throw normalizedV60NotifierGrantRollback;
    }),
    (error: unknown) => error === normalizedV60NotifierGrantRollback,
  );

  const rotatedV60AclTamperRollback = {
    kind: "rotated-v60-acl-tamper",
  } as const;
  await assert.rejects(
    sql.begin(async (transaction) => {
      await transaction`
      ...
      assert.equal(drifted.readiness[0], false);
      assert.equal(drifted.readiness[1], false);
      throw rotatedV60AclTamperRollback;
    }),
    (error: unknown) => error === rotatedV60AclTamperRollback,
  );

  const rotatedV60ConfigTamperRollback = {
    kind: "rotated-v60-config-tamper",
  } as const;
  await assert.rejects(
    sql.begin(async (transaction) => {
      await transaction`
        ALTER FUNCTION app.schema_compatibility_v60() SET work_mem='64kB'
      `;
      const drifted = await readV62Readiness(transaction);
      assert.notEqual(drifted.catalog_digest, expectedCatalogDigest);
      assert.equal(drifted.readiness[0], false);
      assert.equal(drifted.readiness[1], false);
      throw rotatedV60ConfigTamperRollback;
    }),
    (error: unknown) => error === rotatedV60ConfigTamperRollback,
  );

  const unnormalizedV60ConfigRollback = {
    kind: "unnormalized-private-v60-config",
  } as const;
  await assert.rejects(
    sql.begin(async (transaction) => {
      await transaction`
        ALTER FUNCTION app.private_schema_compatibility_journal_v60()
        SET work_mem='64kB'
      `;
      const drifted = await readV62Readiness(transaction);
      assert.notEqual(drifted.catalog_digest, expectedCatalogDigest);
      assert.equal(drifted.readiness[0], false);
      assert.equal(drifted.readiness[1], false);
      throw unnormalizedV60ConfigRollback;
    }),
    (error: unknown) => error === unnormalizedV60ConfigRollback,
  );

  const historicalV60SourceRollback = {
    kind: "historical-v60-private-readiness-source-tamper",
  } as const;
  await assert.rejects(
    sql.begin(async (transaction) => {
      await transaction.unsafe(`
      ...
      assert.notEqual(
        drifted.catalog_digest,
        expectedCatalogDigest,
        "the historical V60 private root is no longer self-excluded in V62",
      );
      assert.equal(drifted.readiness[0], false);
      assert.equal(drifted.readiness[1], false);
      throw historicalV60SourceRollback;
    }),
    (error: unknown) => error === historicalV60SourceRollback,
  );

  const privateV61Catalog = await readFunctionCatalog(
    sql,
    retainedPrivateV61Roots,
  );
  assert.equal(privateV61Catalog.length, retainedPrivateV61Roots.length);
  for (const root of privateV61Catalog) {
    assert.equal(root.owner_name, "periapsis_migrator");
    assert.deepEqual(root.acl_roles, ["periapsis_migrator"]);
    assert.equal(root.runtime_execute_count, 0);
  }

  const [retiredV61Source] = await sql<{ source_hash: string }[]>`
    SELECT encode(sha256(convert_to(prosrc, 'UTF8')), 'hex') AS source_hash
    FROM pg_catalog.pg_proc WHERE oid='app.schema_compatibility_v61()'::regprocedure
  `;
  assert.equal(
    retiredV61Source?.source_hash,
    expectedRetiredSchemaCompatibilityV61SourceHash,
  );

  const normalizedV61GrantRollback = {
    kind: "normalized-v61-grant",
  } as const;
  await assert.rejects(
    sql.begin(async (transaction) => {
      await transaction`
        GRANT EXECUTE ON FUNCTION
          app.ticket_metadata_runtime_schema_readiness_v61()
        TO periapsis_api
      `;
      const drifted = await readV62Readiness(transaction);
      assert.equal(
        drifted.catalog_digest,
        expectedCatalogDigest,
        "the exact V61 ACL rotation is intentionally digest-normalized",
      );
      assert.equal(drifted.readiness[0], true);
      assert.equal(
        drifted.readiness[1],
        false,
        "the public V62 root must independently reject a restored V61 grant",
      );
      throw normalizedV61GrantRollback;
    }),
    (error: unknown) => error === normalizedV61GrantRollback,
  );

  const normalizedV61NotifierGrantRollback = {
    kind: "normalized-v61-notifier-grant",
  } as const;
  await assert.rejects(
    sql.begin(async (transaction) => {
      await transaction`
        GRANT EXECUTE ON FUNCTION app.notification_dispatch_readiness_v61()
        TO periapsis_notifier
      `;
      const drifted = await readV62Readiness(transaction);
      assert.equal(drifted.catalog_digest, expectedCatalogDigest);
      ...
      throw normalizedV61NotifierGrantRollback;
    }),
    (error: unknown) => error === normalizedV61NotifierGrantRollback,
  );

  const rotatedV61AclTamperRollback = {
    kind: "rotated-v61-acl-tamper",
  } as const;
  await assert.rejects(
    sql.begin(async (transaction) => {
      await transaction`
      ...
      assert.equal(drifted.readiness[0], false);
      assert.equal(drifted.readiness[1], false);
      throw rotatedV61AclTamperRollback;
    }),
    (error: unknown) => error === rotatedV61AclTamperRollback,
  );

  const rotatedV61ConfigTamperRollback = {
    kind: "rotated-v61-config-tamper",
  } as const;
  await assert.rejects(
    sql.begin(async (transaction) => {
      await transaction`
        ALTER FUNCTION app.schema_compatibility_v61() SET work_mem='64kB'
      `;
      const drifted = await readV62Readiness(transaction);
      assert.notEqual(drifted.catalog_digest, expectedCatalogDigest);
      assert.equal(drifted.readiness[0], false);
      assert.equal(drifted.readiness[1], false);
      throw rotatedV61ConfigTamperRollback;
    }),
    (error: unknown) => error === rotatedV61ConfigTamperRollback,
  );

  const unnormalizedV61ConfigRollback = {
    kind: "unnormalized-private-v61-config",
  } as const;
  await assert.rejects(
    sql.begin(async (transaction) => {
      await transaction`
        ALTER FUNCTION app.private_schema_compatibility_journal_v61()
        SET work_mem='64kB'
      `;
      const drifted = await readV62Readiness(transaction);
      assert.notEqual(drifted.catalog_digest, expectedCatalogDigest);
      assert.equal(drifted.readiness[0], false);
      assert.equal(drifted.readiness[1], false);
      throw unnormalizedV61ConfigRollback;
    }),
    (error: unknown) => error === unnormalizedV61ConfigRollback,
  );

  const historicalV61SourceRollback = {
    kind: "historical-v61-private-readiness-source-tamper",
  } as const;
  await assert.rejects(
    sql.begin(async (transaction) => {
      await transaction.unsafe(`
      ...
      assert.notEqual(
        drifted.catalog_digest,
        expectedCatalogDigest,
        "the historical V61 private root is no longer self-excluded in V62",
      );
      assert.equal(drifted.readiness[0], false);
      assert.equal(drifted.readiness[1], false);
      throw historicalV61SourceRollback;
    }),
    (error: unknown) => error === historicalV61SourceRollback,
  );
  ...
  await assert.rejects(
    sql.begin(async (transaction) => {
      ...
      assert.equal(
        drifted.readiness[1],
        false,
        "the public V62 root must independently reject a restored V49 grant",
      );
  ...
  await assert.rejects(
    sql.begin(async (transaction) => {
      ...
      assert.notEqual(
        drifted.catalog_digest,
        expectedCatalogDigest,
        "the historical V49 private root is no longer self-excluded in V62",
      );
      assert.equal(drifted.readiness[0], false);
      assert.equal(drifted.readiness[1], false);
  ...
  await assert.rejects(
    sql.begin(async (transaction) => {
      ...
      assert.equal(
        drifted.readiness[1],
        false,
        "the public V62 root must independently reject a restored V48 grant",
      );
  ...
  assert.deepEqual(await readV62Readiness(sql), readiness);
    ...
    sql.begin(async (transaction) => {
      ...
      await transaction.unsafe(`
        ...
        RETURN pg_catalog.sha256(pg_catalog.convert_to(
          'v62-tampered-ticket-namespace', 'UTF8'
        ))
      `);
  ...
  await assert.rejects(
    sql.begin(async (transaction) => {
      await transaction.unsafe(`
        CREATE RULE v62_rewrite_tamper
  ...
  await assert.rejects(
    sql.begin(async (transaction) => {
      await transaction.unsafe(`
        CREATE FUNCTION public.v62_public_function_probe()
  ...
  await assert.rejects(
    sql.begin(async (transaction) => {
      await transaction.unsafe(`
        CREATE SCHEMA v62_evil;
        CREATE FUNCTION v62_evil.alert_status_eq(
          public.alert_status,
          public.alert_status
        ) RETURNS boolean LANGUAGE sql IMMUTABLE
        SET search_path=pg_catalog
      ...
      await transaction.unsafe(`
        CREATE OPERATOR public.= (
          ...
          FUNCTION=v62_evil.alert_status_eq
        )
      `);
      const [after] = await transaction.unsafe<{ result: boolean }[]>(`
        SELECT 'new'::public.alert_status = 'new'::public.alert_status
          AS result /* v62-operator-after */
      `);
  ...
  await assert.rejects(
    sql.begin(async (transaction) => {
      ...
      assert.equal((await readV62Readiness(transaction)).readiness[0], false);
  ...
  await assert.rejects(
    sql.begin(async (transaction) => {
      await transaction.unsafe(`
        CREATE DOMAIN public.v62_domain_probe AS integer
      `);
      const domainOnlyDigest = await readCatalogDigest(transaction);
      await transaction.unsafe(`
        ALTER DOMAIN public.v62_domain_probe
        ADD CONSTRAINT v62_positive CHECK (VALUE > 0)
      `);
  ...
  await assert.rejects(
    sql.begin(async (transaction) => {
      await transaction.unsafe(`
        CREATE FUNCTION public.v62_event_trigger_handler()
      ...
      await transaction.unsafe(`
        CREATE EVENT TRIGGER v62_event_trigger_probe
        ON ddl_command_end
        WHEN TAG IN ('CREATE TABLE')
        EXECUTE FUNCTION public.v62_event_trigger_handler()
      `);
  ...
  await assert.rejects(
    sql.begin(async (transaction) => {
      await transaction.unsafe(`
        CREATE PUBLICATION v62_alert_leak
      ...
      await transaction.unsafe(`
        CREATE PUBLICATION v62_public_schema_leak
      ...
      await transaction.unsafe(`
        CREATE PUBLICATION v62_all_tables_leak FOR ALL TABLES
      `);
      ...
      await transaction.unsafe(`
        ALTER PUBLICATION v62_alert_leak SET (publish='insert')
      `);
      assert.notEqual(
        await readCatalogDigest(transaction),
        allTablesPublicationDigest,
  ...
  await assert.rejects(
    sql.begin(async (transaction) => {
      await transaction.unsafe(`
        CREATE SUBSCRIPTION v62_injection
  ...
  await assert.rejects(
    sql.begin(async (transaction) => {
      await transaction.unsafe(`
        CREATE ROLE v62_alternate_sealer SUPERUSER NOLOGIN;
        ALTER FUNCTION app.private_runtime_login_credential_state_v49()
          OWNER TO v62_alternate_sealer;
        SET ROLE v62_alternate_sealer;
  ...
  await assert.rejects(
    sql.begin(async (transaction) => {
      ...
      await transaction.unsafe(`
        CREATE ROLE v62_alternate_membership_grantor NOLOGIN;
        GRANT periapsis_api TO v62_alternate_membership_grantor
          WITH ADMIN OPTION
      `);
      ...
      await transaction.unsafe(`
        GRANT periapsis_api TO periapsis_api_login
          WITH ADMIN FALSE, INHERIT TRUE, SET TRUE
          GRANTED BY v62_alternate_membership_grantor
      `);
      const [edge] = await transaction<{ grantor: string }[]>`
        ...
        WHERE granted_role.rolname='periapsis_api'
          AND member_role.rolname='periapsis_api_login'
      `;
      assert.equal(edge?.grantor, "v62_alternate_membership_grantor");
      assert.equal(
        await readCatalogDigest(transaction),
        withOriginalGrantor,
  ...
  await assert.rejects(
    sql.begin(async (transaction) => {
      await transaction.unsafe(`
        CREATE ROLE periapsis_v62_membership_granted NOLOGIN;
        CREATE ROLE periapsis_v62_membership_member NOLOGIN;
        GRANT periapsis_v62_membership_granted
          TO periapsis_v62_membership_member
          WITH ADMIN FALSE, INHERIT TRUE, SET TRUE
      `);
  ...
  await assert.rejects(
    sql.begin(async (transaction) => {
      await transaction.unsafe(`
        CREATE ROLE periapsis_v62_acl_grantor NOLOGIN;
        CREATE ROLE periapsis_v62_acl_grantee NOLOGIN;
        GRANT USAGE ON SCHEMA app TO periapsis_v62_acl_grantor;
        CREATE FUNCTION app.v62_acl_grantor_probe()
        RETURNS boolean LANGUAGE sql IMMUTABLE
        SET search_path=pg_catalog
        AS 'SELECT true';
        ALTER FUNCTION app.v62_acl_grantor_probe()
          OWNER TO periapsis_migrator;
        REVOKE ALL ON FUNCTION app.v62_acl_grantor_probe() FROM PUBLIC;
        GRANT EXECUTE ON FUNCTION app.v62_acl_grantor_probe()
          TO periapsis_v62_acl_grantor WITH GRANT OPTION;
        SET ROLE periapsis_v62_acl_grantor;
        GRANT EXECUTE ON FUNCTION app.v62_acl_grantor_probe()
          TO periapsis_v62_acl_grantee;
        RESET ROLE
      `);
      const delegated = await readCatalogDigest(transaction);
      await transaction.unsafe(`
        SET ROLE periapsis_v62_acl_grantor;
        REVOKE EXECUTE ON FUNCTION app.v62_acl_grantor_probe()
          FROM periapsis_v62_acl_grantee;
        RESET ROLE;
  ...
  await assert.rejects(
    sql.begin(async (transaction) => {
      await transaction.unsafe(`
        CREATE ROLE v62_external_default_acl_owner NOLOGIN;
        ALTER DEFAULT PRIVILEGES FOR ROLE v62_external_default_acl_owner
  ...
  await assert.rejects(
    sql.begin(async (transaction) => {
      await transaction`
        CREATE ROLE v62_alternate_database_owner SUPERUSER NOLOGIN
      `;
      const [alter] = await transaction<{ statement: string }[]>`
        SELECT pg_catalog.format(
          'ALTER DATABASE %I OWNER TO v62_alternate_database_owner',
          pg_catalog.current_database()
        ) AS statement
      `;
  ...
  await assert.rejects(
    sql.begin(async (transaction) => {
      await transaction.unsafe(`
        ALTER ROLE ALL SET application_name = 'v62-global-role-tamper'
      `);
      const drifted = await readV62Readiness(transaction);
      assert.notEqual(drifted.catalog_digest, expectedCatalogDigest);
      assert.equal(drifted.readiness[0], false);
  ...
  await assert.rejects(
    sql.begin(async (transaction) => {
      ...
      assert.equal(
        await readCatalogDigest(transaction),
        expectedCatalogDigest,
        "the exact private V62 root remains self-excluded from its own digest",
      );
      assert.equal(
        await readNotifierSourceAttestation(transaction),
        false,
        "the notifier must independently source-attest its V62 trust chain",
      );
  ...
  const sameNameOverloadRollback = {
    kind: "same-name-v62-overload",
  } as const;
  await assert.rejects(
    sql.begin(async (transaction) => {
      await transaction.unsafe(`
        CREATE FUNCTION app.release_runtime_schema_readiness_v62(text)
        RETURNS boolean LANGUAGE sql IMMUTABLE
        SET search_path=pg_catalog
        AS 'SELECT true'
      `);
      await transaction`
        ALTER FUNCTION app.release_runtime_schema_readiness_v62(text)
        OWNER TO periapsis_migrator
      `;
      await transaction`
        REVOKE ALL ON FUNCTION app.release_runtime_schema_readiness_v62(text)
      ...
      assert.notEqual(
        drifted.catalog_digest,
        expectedCatalogDigest,
        "a same-name overload must not inherit the exact V62 self-exclusion",
      );
      assert.equal(drifted.readiness[0], false);
      assert.equal(drifted.readiness[1], false);
