...
import {
  ...
  expectedNotificationDispatchReadinessV56SourceHash,
  expectedPrivateReleaseRuntimeDependencySurfaceHashV56SourceHash,
  expectedPrivateReleaseRuntimeReadinessV56SourceHash,
  expectedReleaseRuntimeReadinessV56SourceHash,
  ...
  expectedSchemaCompatibilityV56SourceHash,
...
const v56Migration = await readFile(
  resolve(migrationsRoot, "0243_v56_compatibility.sql"),
  "utf8",
);
const expectedCatalogDigest =
  /private_release_runtime_dependency_surface_hash_v56\(\)<>\s*'([0-9a-f]{64})'/u.exec(
    v56Migration,
  )?.[1];
assert(
  expectedCatalogDigest,
  "0243 must pin the exact V56 release catalog digest",
);
assert.notEqual(
  expectedCatalogDigest,
  "0".repeat(64),
  "0243 must contain the independently derived V56 digest, not a placeholder",
);
assert.equal(
  expectedCatalogDigest,
...
assert.equal(expectedMigrationCreatedAt, 1_788_805_357_439);
...
async function readV56Readiness(
  sql: Sql | postgres.TransactionSql,
): Promise<ReadinessRow> {
  const [row] = await sql<ReadinessRow[]>`
    SELECT app.private_release_runtime_dependency_surface_hash_v56()
             AS catalog_digest,
           ARRAY[
           ...
           ]::boolean[] || app.api_runtime_schema_readiness_v56()
             || app.worker_runtime_schema_readiness_v56() AS readiness
  `;
  assert(row, "V56 readiness returned no row");
...
async function readCatalogDigest(
  ...
  const [row] = await sql<{ digest: string }[]>`
    SELECT app.private_release_runtime_dependency_surface_hash_v56()
             AS digest
  `;
  assert(row, "V56 catalog digest returned no row");
  return row.digest;
}
...
async function readNotifierSourceAttestation(
  ...
  `;
  assert(row, "notifier V56 source attestation returned no row");
...
assert(
  ...
  "PERIAPSIS_SCHEMA_COMPATIBILITY_V56_SECURITY_TEST_DATABASE_URL is required",
);
...
try {
  ...
  `;
  assert(compatibility, "schema_compatibility_v56 returned no row");
  assert.deepEqual(
    {
  ...
  const readiness = await readV56Readiness(sql);
  assert.equal(readiness.catalog_digest, expectedCatalogDigest);
  assert.equal(readiness.readiness.length, 26);
  assert(!readiness.readiness.includes(false), "a V56 readiness root failed");
  ...
  assert.deepEqual(notifierCatalog, [
    ...
    },
    {
      ...
      function_name: "notification_dispatch_readiness_v56",
      owner_name: "periapsis_notification_readiness_owner",
  ...
  await assert.rejects(
    sql.begin(async (transaction) => {
      await transaction`
        ...
        TO periapsis_api
      `;
      const drifted = await readV56Readiness(transaction);
      assert.equal(
      ...
      assert.equal(
        drifted.readiness[1],
        false,
        "the public V56 root must independently reject a restored V50 grant",
      );
  ...
  await assert.rejects(
    sql.begin(async (transaction) => {
      ...
      assert.notEqual(
        drifted.catalog_digest,
        expectedCatalogDigest,
        "the historical V50 private root is no longer self-excluded in V56",
      );
      assert.equal(drifted.readiness[0], false);
      assert.equal(drifted.readiness[1], false);
  ...
  await assert.rejects(
    sql.begin(async (transaction) => {
      ...
      assert.equal(
        drifted.readiness[1],
        false,
        "the public V56 root must independently reject a restored V51 grant",
      );
  ...
  await assert.rejects(
    sql.begin(async (transaction) => {
      ...
      assert.notEqual(
        drifted.catalog_digest,
        expectedCatalogDigest,
        "the historical V51 private root is no longer self-excluded in V56",
      );
      assert.equal(drifted.readiness[0], false);
      assert.equal(drifted.readiness[1], false);
  ...
  await assert.rejects(
    sql.begin(async (transaction) => {
      ...
      assert.equal(
        drifted.readiness[1],
        false,
        "the public V56 root must independently reject a restored V52 grant",
      );
  ...
  await assert.rejects(
    sql.begin(async (transaction) => {
      ...
      assert.notEqual(
        drifted.catalog_digest,
        expectedCatalogDigest,
        "the historical V52 private root is no longer self-excluded in V56",
      );
      assert.equal(drifted.readiness[0], false);
      assert.equal(drifted.readiness[1], false);
  ...
  await assert.rejects(
    sql.begin(async (transaction) => {
      ...
      assert.equal(
        drifted.readiness[1],
        false,
        "the public V56 root must independently reject a restored V53 grant",
      );
  ...
  await assert.rejects(
    sql.begin(async (transaction) => {
      ...
      assert.notEqual(
        drifted.catalog_digest,
        expectedCatalogDigest,
        "the historical V53 private root is no longer self-excluded in V56",
      );
      assert.equal(drifted.readiness[0], false);
      assert.equal(drifted.readiness[1], false);
  ...
  await assert.rejects(
    sql.begin(async (transaction) => {
      ...
      assert.equal(
        drifted.readiness[1],
        false,
        "the public V56 root must independently reject a restored V54 grant",
      );
  ...
  await assert.rejects(
    sql.begin(async (transaction) => {
      ...
      assert.notEqual(
        drifted.catalog_digest,
        expectedCatalogDigest,
        "the historical V54 private root is no longer self-excluded in V56",
      );
      assert.equal(drifted.readiness[0], false);
      assert.equal(drifted.readiness[1], false);
  ...
  await assert.rejects(
    sql.begin(async (transaction) => {
      ...
      assert.equal(
        drifted.readiness[1],
        false,
        "the public V56 root must independently reject a restored V55 grant",
      );
  ...
  await assert.rejects(
    sql.begin(async (transaction) => {
      ...
      assert.notEqual(
        drifted.catalog_digest,
        expectedCatalogDigest,
        "the historical V55 private root is no longer self-excluded in V56",
      );
      assert.equal(drifted.readiness[0], false);
      assert.equal(drifted.readiness[1], false);
  ...
  await assert.rejects(
    sql.begin(async (transaction) => {
      ...
      assert.equal(
        drifted.readiness[1],
        false,
        "the public V56 root must independently reject a restored V49 grant",
      );
  ...
  await assert.rejects(
    sql.begin(async (transaction) => {
      ...
      assert.notEqual(
        drifted.catalog_digest,
        expectedCatalogDigest,
        "the historical V49 private root is no longer self-excluded in V56",
      );
      assert.equal(drifted.readiness[0], false);
      assert.equal(drifted.readiness[1], false);
  ...
  await assert.rejects(
    sql.begin(async (transaction) => {
      ...
      assert.equal(
        drifted.readiness[1],
        false,
        "the public V56 root must independently reject a restored V48 grant",
      );
  ...
  assert.deepEqual(await readV56Readiness(sql), readiness);
    ...
    sql.begin(async (transaction) => {
      ...
      await transaction.unsafe(`
        ...
        RETURN pg_catalog.sha256(pg_catalog.convert_to(
          'v56-tampered-ticket-namespace', 'UTF8'
        ))
      `);
  ...
  await assert.rejects(
    sql.begin(async (transaction) => {
      await transaction.unsafe(`
        CREATE RULE v56_rewrite_tamper
  ...
  await assert.rejects(
    sql.begin(async (transaction) => {
      await transaction.unsafe(`
        CREATE FUNCTION public.v56_public_function_probe()
  ...
  await assert.rejects(
    sql.begin(async (transaction) => {
      await transaction.unsafe(`
        CREATE SCHEMA v56_evil;
        CREATE FUNCTION v56_evil.alert_status_eq(
          public.alert_status,
          public.alert_status
        ) RETURNS boolean LANGUAGE sql IMMUTABLE
        SET search_path=pg_catalog
      ...
      await transaction.unsafe(`
        CREATE OPERATOR public.= (
          ...
          FUNCTION=v56_evil.alert_status_eq
        )
      `);
      const [after] = await transaction.unsafe<{ result: boolean }[]>(`
        SELECT 'new'::public.alert_status = 'new'::public.alert_status
          AS result /* v56-operator-after */
      `);
  ...
  await assert.rejects(
    sql.begin(async (transaction) => {
      ...
      assert.equal((await readV56Readiness(transaction)).readiness[0], false);
  ...
  await assert.rejects(
    sql.begin(async (transaction) => {
      await transaction.unsafe(`
        CREATE DOMAIN public.v56_domain_probe AS integer
      `);
      const domainOnlyDigest = await readCatalogDigest(transaction);
      await transaction.unsafe(`
        ALTER DOMAIN public.v56_domain_probe
        ADD CONSTRAINT v56_positive CHECK (VALUE > 0)
      `);
  ...
  await assert.rejects(
    sql.begin(async (transaction) => {
      await transaction.unsafe(`
        CREATE FUNCTION public.v56_event_trigger_handler()
      ...
      await transaction.unsafe(`
        CREATE EVENT TRIGGER v56_event_trigger_probe
        ON ddl_command_end
        WHEN TAG IN ('CREATE TABLE')
        EXECUTE FUNCTION public.v56_event_trigger_handler()
      `);
  ...
  await assert.rejects(
    sql.begin(async (transaction) => {
      await transaction.unsafe(`
        CREATE PUBLICATION v56_alert_leak
      ...
      await transaction.unsafe(`
        CREATE PUBLICATION v56_public_schema_leak
      ...
      await transaction.unsafe(`
        CREATE PUBLICATION v56_all_tables_leak FOR ALL TABLES
      `);
      ...
      await transaction.unsafe(`
        ALTER PUBLICATION v56_alert_leak SET (publish='insert')
      `);
      assert.notEqual(
        await readCatalogDigest(transaction),
        allTablesPublicationDigest,
  ...
  await assert.rejects(
    sql.begin(async (transaction) => {
      await transaction.unsafe(`
        CREATE SUBSCRIPTION v56_injection
  ...
  await assert.rejects(
    sql.begin(async (transaction) => {
      await transaction.unsafe(`
        CREATE ROLE v56_alternate_sealer SUPERUSER NOLOGIN;
        ALTER FUNCTION app.private_runtime_login_credential_state_v49()
          OWNER TO v56_alternate_sealer;
        SET ROLE v56_alternate_sealer;
  ...
  await assert.rejects(
    sql.begin(async (transaction) => {
      ...
      await transaction.unsafe(`
        CREATE ROLE v56_alternate_membership_grantor NOLOGIN;
        GRANT periapsis_api TO v56_alternate_membership_grantor
          WITH ADMIN OPTION
      `);
      ...
      await transaction.unsafe(`
        GRANT periapsis_api TO periapsis_api_login
          WITH ADMIN FALSE, INHERIT TRUE, SET TRUE
          GRANTED BY v56_alternate_membership_grantor
      `);
      const [edge] = await transaction<{ grantor: string }[]>`
        ...
        WHERE granted_role.rolname='periapsis_api'
          AND member_role.rolname='periapsis_api_login'
      `;
      assert.equal(edge?.grantor, "v56_alternate_membership_grantor");
      assert.equal(
        await readCatalogDigest(transaction),
        withOriginalGrantor,
  ...
  await assert.rejects(
    sql.begin(async (transaction) => {
      await transaction.unsafe(`
        CREATE ROLE periapsis_v56_membership_granted NOLOGIN;
        CREATE ROLE periapsis_v56_membership_member NOLOGIN;
        GRANT periapsis_v56_membership_granted
          TO periapsis_v56_membership_member
          WITH ADMIN FALSE, INHERIT TRUE, SET TRUE
      `);
  ...
  await assert.rejects(
    sql.begin(async (transaction) => {
      await transaction.unsafe(`
        CREATE ROLE periapsis_v56_acl_grantor NOLOGIN;
        CREATE ROLE periapsis_v56_acl_grantee NOLOGIN;
        GRANT USAGE ON SCHEMA app TO periapsis_v56_acl_grantor;
        CREATE FUNCTION app.v56_acl_grantor_probe()
        RETURNS boolean LANGUAGE sql IMMUTABLE
        SET search_path=pg_catalog
        AS 'SELECT true';
        ALTER FUNCTION app.v56_acl_grantor_probe()
          OWNER TO periapsis_migrator;
        REVOKE ALL ON FUNCTION app.v56_acl_grantor_probe() FROM PUBLIC;
        GRANT EXECUTE ON FUNCTION app.v56_acl_grantor_probe()
          TO periapsis_v56_acl_grantor WITH GRANT OPTION;
        SET ROLE periapsis_v56_acl_grantor;
        GRANT EXECUTE ON FUNCTION app.v56_acl_grantor_probe()
          TO periapsis_v56_acl_grantee;
        RESET ROLE
      `);
      const delegated = await readCatalogDigest(transaction);
      await transaction.unsafe(`
        SET ROLE periapsis_v56_acl_grantor;
        REVOKE EXECUTE ON FUNCTION app.v56_acl_grantor_probe()
          FROM periapsis_v56_acl_grantee;
        RESET ROLE;
  ...
  await assert.rejects(
    sql.begin(async (transaction) => {
      await transaction.unsafe(`
        CREATE ROLE v56_external_default_acl_owner NOLOGIN;
        ALTER DEFAULT PRIVILEGES FOR ROLE v56_external_default_acl_owner
  ...
  await assert.rejects(
    sql.begin(async (transaction) => {
      await transaction`
        CREATE ROLE v56_alternate_database_owner SUPERUSER NOLOGIN
      `;
      const [alter] = await transaction<{ statement: string }[]>`
        SELECT pg_catalog.format(
          'ALTER DATABASE %I OWNER TO v56_alternate_database_owner',
          pg_catalog.current_database()
        ) AS statement
      `;
  ...
  await assert.rejects(
    sql.begin(async (transaction) => {
      await transaction.unsafe(`
        ALTER ROLE ALL SET application_name = 'v56-global-role-tamper'
      `);
      const drifted = await readV56Readiness(transaction);
      assert.notEqual(drifted.catalog_digest, expectedCatalogDigest);
      assert.equal(drifted.readiness[0], false);
  ...
  await assert.rejects(
    sql.begin(async (transaction) => {
      ...
      assert.equal(
        await readCatalogDigest(transaction),
        expectedCatalogDigest,
        "the exact private V56 root remains self-excluded from its own digest",
      );
      assert.equal(
        await readNotifierSourceAttestation(transaction),
        false,
        "the notifier must independently source-attest its V56 trust chain",
      );
  ...
  const sameNameOverloadRollback = {
    kind: "same-name-v56-overload",
  } as const;
  await assert.rejects(
    sql.begin(async (transaction) => {
      await transaction.unsafe(`
        CREATE FUNCTION app.release_runtime_schema_readiness_v56(text)
        RETURNS boolean LANGUAGE sql IMMUTABLE
        SET search_path=pg_catalog
        AS 'SELECT true'
      `);
      await transaction`
        ALTER FUNCTION app.release_runtime_schema_readiness_v56(text)
        OWNER TO periapsis_migrator
      `;
      await transaction`
        REVOKE ALL ON FUNCTION app.release_runtime_schema_readiness_v56(text)
      ...
      assert.notEqual(
        drifted.catalog_digest,
        expectedCatalogDigest,
        "a same-name overload must not inherit the exact V56 self-exclusion",
      );
      assert.equal(drifted.readiness[0], false);
      assert.equal(drifted.readiness[1], false);
