  ...
  await assert.rejects(
    admin.begin(async (transaction) => {
      ...
      const [tampered] = await transaction<{ ready: boolean }[]>`
        SELECT app.release_runtime_schema_readiness_v49()
                 AS ready
      `;
      assert.equal(
        tampered?.ready,
...
async function verifyCompatibilityAndAcl(): Promise<void> {
  ...
  >`
    SELECT
      current.applied_count::integer AS v49,
      ...
      app.release_runtime_schema_readiness_v49() AS ready,
      app.platform_oidc_direct_runtime_schema_readiness_v49() AS "oidcReady"
    FROM app.schema_compatibility_v49() AS current
    CROSS JOIN app.schema_compatibility_v48() AS predecessor
    CROSS JOIN app.schema_compatibility_v40() AS legacy
  `;
  assert.deepEqual(compatibility, {
    v49: expectedMigrationCount,
