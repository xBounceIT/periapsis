...
assert.deepEqual(
  ...
  ["0246_sla_notification_contact_runtime", "0247_v58_compatibility"],
);
...
assert(v58CatalogDigest, "0247 must pin the V58 catalog digest");
assert.notEqual(
  v58CatalogDigest,
  "0".repeat(64),
...
try {
  ...
  try {
    ...
    throw new Error(
      "ordinary SAML admission/runtime proof failed after V57 -> V58 upgrade",
    );
  }
  await assertSealedV58();
  assert.deepEqual(
