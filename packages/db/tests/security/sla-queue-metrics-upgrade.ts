...
const databaseUrl =
  process.env.PERIAPSIS_SLA_QUEUE_METRICS_UPGRADE_TEST_DATABASE_URL;
if (databaseUrl === undefined || databaseUrl.trim() === "") {
  throw new Error(
    "PERIAPSIS_SLA_QUEUE_METRICS_UPGRADE_TEST_DATABASE_URL must name an isolated empty PostgreSQL 18 database whose cluster has no periapsis_* roles",
  );
}
...
const predecessorIndex = 134;
const predecessorEntries = journal.entries.slice(0, predecessorIndex + 1);
const predecessor = expectedMigrations[predecessorIndex];
assert.equal(predecessorEntries.length, 135);
assert.equal(predecessor?.tag, "0134_federated_authentication_readiness");
assert.equal(predecessor.createdAt, 1787732438000);
assert.equal(
  predecessor.hash,
...
const stageRoot = await mkdtemp(
  join(tmpdir(), "periapsis-sla-queue-metrics-rolling-0134-"),
);
...
try {
  ...
  >`
    SELECT current_projection.applied_count::integer AS current_count,
           ...
           predecessor_projection.latest_created_at::text
             AS predecessor_latest,
           predecessor_projection.latest_hash AS predecessor_hash,
           predecessor_projection.migration_fingerprint
             AS predecessor_fingerprint,
           retired_projection.applied_count::integer AS retired_count,
           app.release_runtime_schema_readiness_v53() AS release_ready,
    ...
    FROM app.schema_compatibility_v53() AS current_projection
  ...
  const workerResult = await sql.begin(async (transaction) => {
    await transaction.unsafe('SET LOCAL ROLE "periapsis_worker"');
    return {
      value: await transaction<
        { pending_jobs: string; oldest_pending_micros: string }[]
      >`
        SELECT pending_jobs::text, oldest_pending_micros::text
        FROM app.read_sla_evaluation_queue_metrics_v1()
      `,
    };
  });
  const [emptyMetrics] = workerResult.value;
  assert.deepEqual(emptyMetrics, {
    pending_jobs: "0",
    oldest_pending_micros: "0",
  });
  assert.equal(workerResult.value.length, 1);
