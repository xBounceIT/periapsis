...
async function main(): Promise<void> {
  ...
  >`
    SELECT app.release_runtime_schema_readiness_v53() AS watcher_ready,
  ...
  await assert.rejects(
    primary.begin(async (transaction) => {
      ...
      assert.equal(
        tampered?.ready,
        false,
        "V53 release readiness accepted a helper without Unicode edge whitespace",
      );
  ...
  await primary.begin(async (transaction) => {
    ...
    assert.equal(
      tampered?.ready,
      false,
      "V53 release readiness accepted a watcher snapshot constraint without bidi controls",
    );
    await transaction`
      ALTER TABLE public.ticket_watcher_events
        DROP CONSTRAINT ticket_watcher_events_display_name_check
