...
assert.deepEqual(
  ...
  ["0240_tenant_profile_projections", "0241_v55_compatibility"],
);
...
assert(v55CatalogDigest, "0241 must pin the V55 catalog digest");
assert.notEqual(
  v55CatalogDigest,
  "0".repeat(64),
...
try {
  ...
  try {
    ...
    throw new Error(
      "ordinary SAML admission/runtime proof failed after V54 -> V55 upgrade",
    );
  }
  await assertSealedV55();
  assert.deepEqual(
