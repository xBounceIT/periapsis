...
assert.deepEqual(
  ...
  ["0250_ticket_export_authority", "0251_v60_compatibility"],
);
...
assert(v60CatalogDigest, "0251 must pin the V60 catalog digest");
assert.notEqual(
  v60CatalogDigest,
  "0".repeat(64),
...
try {
  ...
  try {
    ...
    throw new Error(
      "ordinary SAML admission/runtime proof failed after V59 -> V60 upgrade",
    );
  }
  await assertSealedV60();
  assert.deepEqual(
