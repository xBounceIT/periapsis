...
const v61Migration = source(
  "packages/db/migrations/0253_v61_compatibility.sql",
);
...
describe("SMTP runtime assurance contract", () => {
  ...
  it("roots readiness in exact owner, ACL, RLS, and direct-audit revocation checks", () => {
    ...
    expect(v61Migration).toMatch(
      /GRANT EXECUTE ON FUNCTION app\.notification_dispatch_readiness_v61\(\)\s+TO periapsis_migrator,periapsis_notifier;/,
    );
    expect(v61Migration).toContain(
