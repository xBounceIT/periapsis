...
const v58Migration = source(
  "packages/db/migrations/0247_v58_compatibility.sql",
);
...
describe("SMTP runtime assurance contract", () => {
  ...
  it("roots readiness in exact owner, ACL, RLS, and direct-audit revocation checks", () => {
    ...
    expect(v58Migration).toMatch(
      /GRANT EXECUTE ON FUNCTION app\.notification_dispatch_readiness_v58\(\)\s+TO periapsis_migrator,periapsis_notifier;/,
    );
    expect(v58Migration).toContain(
