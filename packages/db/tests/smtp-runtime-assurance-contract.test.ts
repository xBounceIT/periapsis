...
const v62Migration = source(
  "packages/db/migrations/0255_v62_compatibility.sql",
);
...
describe("SMTP runtime assurance contract", () => {
  ...
  it("roots readiness in exact owner, ACL, RLS, and direct-audit revocation checks", () => {
    ...
    expect(v62Migration).toMatch(
      /GRANT EXECUTE ON FUNCTION app\.notification_dispatch_readiness_v62\(\)\s+TO periapsis_migrator,periapsis_notifier;/,
    );
    expect(v62Migration).toContain(
