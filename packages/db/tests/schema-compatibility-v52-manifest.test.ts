...
const repairTag = "0234_service_readiness_aggregation";
const sealTag = "0235_v52_compatibility";
const expectedCount = 236;
const currentRoots = [
...
describe("schema compatibility V52 manifest", () => {
  it("pins the exact complete 0000-0235 inventory and every packaged SQL byte hash", () => {
    ...
    expect(manifest.expectedMigrationCreatedAt).toBe(1_788_707_601_308);
  ...
  it("pins the same current catalog in both upgrade paths and the runtime suite", () => {
    const catalogDigest =
      /private_release_runtime_dependency_surface_hash_v52\(\)<>\s*'([a-f0-9]{64})'/u.exec(
        migration(sealTag),
      )?.[1];
    ...
    for (const name of [
      ...
      "schema-compatibility-v52-runtime.ts",
    ]) {
      ...
      const pinned =
        /assert\.equal\(\s*(?:v52CatalogDigest|expectedCatalogDigest),\s*"([a-f0-9]{64})",\s*\)/u.exec(
          runtimeSource,
        )?.[1];
  ...
  it("keeps both custom migration snapshots structurally identical to V51", () => {
    const snapshots = ["0233", "0234", "0235"].map((version) => {
      const value: unknown = JSON.parse(
  ...
  it("source-attests every V52 root and refuses an unsealed zero dependency digest", () => {
    const seal = migration(sealTag);
    for (const [name, constant] of currentRoots) {
      expect(manifest).toHaveProperty(
        `expected${constant}V52SourceHash`,
        sha256(readRoutine(seal, `${name}_v52`).body),
      );
    }
    const sealer = readRoutine(seal, "seal_schema_compatibility_manifest");
    expect(manifest.expectedSealSchemaCompatibilityManifestV52SourceHash).toBe(
      sha256(sealer.body),
    );
    expect(sealer.body).toContain("p_expected_count IS DISTINCT FROM 236");
    expect(sealer.body).toContain(
      "p_expected_latest_created_at IS DISTINCT FROM 1788707601308",
    );
    expect(sealer.body).toContain("(:[0-9]+@[0-9a-f]{64}){235}$");
    const readiness = readRoutine(
      seal,
      "private_release_runtime_schema_readiness_v52",
    );
    const digest =
      /app\.private_release_runtime_dependency_surface_hash_v52\(\)<>\s*'([0-9a-f]{64})'/u.exec(
        readiness.body,
      )?.[1];
  ...
  it("preserves the complete catalog transcript except exact V51 retirement rows and four named self exclusions", () => {
    const predecessor = readRoutine(
      migration("0233_v51_compatibility"),
  ...
  it("normalizes only the twelve exact V51 ACL pairs and three actual V51 root configuration states", () => {
    const predecessor = migration("0233_v51_compatibility");
    const seal = migration(sealTag);
    ...
    expect(sha256(fingerprint)).toBe(
      "8a24d69b94b137c282c79a5161104913fe323a8fbabdbe589fc75a7bae52033b",
    );
    ...
    expect(seal).toMatch(
      /ALTER FUNCTION app\.schema_compatibility_v51\(\)\s+SET app\.schema_compatibility_fingerprint='RETIRED'/u,
    );
    expect(seal).not.toMatch(/DROP FUNCTION app\.[a-z_]+_v(?:49|5[01])\(/u);
    const release = readRoutine(
      seal,
      "release_runtime_schema_readiness_v52",
    ).body;
    expect(release).toContain("SELECT count(*)=8 AND coalesce(bool_and(");
    for (let version = 44; version <= 51; version++) {
      expect(release).toContain(
        `'app.schema_compatibility_v${version}()'::regprocedure`,
      );
    }
    expect(manifest.expectedRetiredSchemaCompatibilityV51SourceHash).toBe(
      sha256(readRoutine(predecessor, "schema_compatibility_v51").body),
    );
  });

  it("wires the serving API, worker, notifier and canonical migration runner to V52", () => {
    for (const path of [
      "services/api/internal/postgres/health.go",
      "services/worker/internal/postgres/health.go",
    ]) {
      const serving = source(path);
      expect(serving, path).toContain("from app.schema_compatibility_v52()");
    ...
    expect(notifier).toContain("expectedSchemaCompatibilityV52SourceHash");
  ...
  it("keeps the selected non-upgrade runtime proofs anchored to the current V52 root", () => {
    ...
    expect(selectedScripts).toContain("test:security:schema-compatibility-v52");
    ...
    expect(files).toContain(
      "tests/security/schema-compatibility-v52-runtime.ts",
    );
    for (const file of files) {
      const runtime = source(`packages/db/${file}`);
      const historicalCalls = [
        ...runtime.matchAll(/app\.schema_compatibility_v([0-9]+)\(\)/gu),
      ].filter((match) => Number(match[1]) < 52);
      if (historicalCalls.length === 0) continue;
      expect(runtime, file).toContain("app.schema_compatibility_v52()");
      expect(runtime, file).not.toMatch(
        /FROM\s+app\.schema_compatibility_v(?:[0-4]?[0-9]|5[01])\(\)\s+AS\s+current(?:_projection)?\b/iu,
      );
      expect(runtime, file).not.toMatch(
        /SELECT\s+applied_count[\s\S]{0,160}?FROM\s+app\.schema_compatibility_v(?:[0-4]?[0-9]|5[01])\(\)/iu,
      );
    }
  });
});
