...
DECLARE
  ...
  migration_0081_rows bigint;
BEGIN
  EXECUTE $query$
    SELECT count(*)::bigint, max(migration.created_at)::bigint,
           count(*) FILTER (
             WHERE migration.created_at = 1787659623982
           )::bigint
    FROM drizzle.__drizzle_migrations AS migration
  $query$ INTO journal_count, journal_latest_created_at, migration_0081_rows;
  IF journal_count = 82
     AND journal_latest_created_at = 1787659623982
     AND migration_0081_rows = 1 THEN
...
-- v12 is the sole rolling predecessor. It projects only its sealed 80-row
-- prefix after the complete v13 journal and manifest have been verified.
...
BEGIN
  ...
  IF full_count = 82
     AND full_latest_created_at = 1787659623982
...
BEGIN
  ...
  IF p_expected_count IS DISTINCT FROM 82
     OR p_expected_latest_created_at IS DISTINCT FROM 1787659623982
     ...
     OR cardinality(fingerprint_entries) IS DISTINCT FROM 82
     OR fingerprint_entries[82] IS DISTINCT FROM (
       p_expected_latest_created_at::text || '@' || p_expected_latest_hash
     ) THEN
    RAISE EXCEPTION 'invalid schema compatibility v13 manifest'
      USING ERRCODE = '22023';
  END IF;
  ...
  IF fingerprint_created_at IS DISTINCT FROM ARRAY[
    ...
    1787659622481, 1787659623982
  ]::bigint[] THEN
    RAISE EXCEPTION 'invalid schema compatibility v13 timestamp sequence'
      USING ERRCODE = '22023';
  END IF;
  ...
  IF actual_count IS DISTINCT FROM p_expected_count
    ...
    RAISE EXCEPTION 'schema compatibility v13 manifest does not match the applied journal'
      USING ERRCODE = '55000';
  END IF;
  ...
  EXECUTE $statement$
    ALTER FUNCTION app.schema_compatibility_v12()
      SET app.schema_compatibility_fingerprint FROM CURRENT
  $statement$;
  expected_predecessor_hash := split_part(fingerprint_entries[80], '@', 2);
  expected_predecessor_fingerprint := array_to_string(
    fingerprint_entries[1:80], ':'
  );
  SELECT compatibility.applied_count, compatibility.latest_created_at,
         compatibility.latest_hash, compatibility.migration_fingerprint
  ...
  IF predecessor_count IS DISTINCT FROM 80
     OR predecessor_latest_created_at IS DISTINCT FROM 1787658434202
    ...
    RAISE EXCEPTION 'sealed schema compatibility v12 prefix is not exact'
      USING ERRCODE = '55000';
  END IF;
  ...
  IF retired_count IS DISTINCT FROM 0 THEN
    RAISE EXCEPTION 'schema compatibility v11 must be retired'
      USING ERRCODE = '55000';
  END IF;
END;
$function$;--> statement-breakpoint
...
DO $identity_sync_access_grant_readiness_assertions$
DECLARE
...
BEGIN
  FOR expected_function IN
    SELECT * FROM (VALUES
      ...
      ('app.claim_tenant_ldap_sync_observation_planning_v2(uuid,uuid,uuid,bytea,bigint,integer[],bytea[])', false),
      ('app.claim_tenant_ldap_sync_observation_planning_v3(uuid,uuid,uuid,bytea,bigint,integer[],bytea[])', true),
  ...
  LOOP
    ...
    IF function_oid IS NULL THEN
      RAISE EXCEPTION 'required LDAP sync ABI % is absent',
    ...
    IF function_owner IS DISTINCT FROM 'periapsis_migrator'
      ...
      RAISE EXCEPTION 'LDAP sync ABI % boundary is not exact',
        expected_function.signature USING ERRCODE = '55000';
    END IF;
  END LOOP;
END;
$identity_sync_access_grant_readiness_assertions$;
