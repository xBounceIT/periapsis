-- Rotate current readiness after the operator-team hardening. New replicas
-- bind the complete 33-row timestamp@hash journal through v5. The immediately
-- preceding 30-row release remains ready through v4 only after the canonical
-- runner seals this exact journal; v3 is retired explicitly.
...
REVOKE ALL ON FUNCTION "app"."schema_compatibility_v5"() FROM PUBLIC;--> statement-breakpoint
...
BEGIN
  ...
  $query$
  INTO migration_0032_rows;

  IF journal_count = 33
     AND journal_latest_created_at = 1787592230466
    ...
    RETURN QUERY EXECUTE $query$
      ...
      SELECT
        ...
        (
          ...
          WHERE latest_prefix_migration.migration_ordinal = 30
        ) AS latest_hash,
        string_agg(
      ...
      WHERE migration.migration_ordinal <= 30
    $query$;
    RETURN;
  END IF;
