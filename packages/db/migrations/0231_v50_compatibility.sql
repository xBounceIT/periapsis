-- V50 seals the complete 0000-0231 prefix, including nullable-tenant logout.
-- The catalog digest is derived from the canonical migrations on PostgreSQL
-- 18.6 UTF8/C. Only the normal, source-attested migration runner can seal it.
-- All V49 function sources stay in the catalog transcript. Only the four exact
-- V50 self-referential signatures are excluded and must be source-attested by
-- the generated manifest and runtime callers. Existing transcript-format
-- markers and historical normalization remain unchanged; twelve exact V49 ACL
-- transitions and three exact V49 root config states are added, not wildcards.
-- The existing V49 credential probe is reused without widening its privilege.
CREATE FUNCTION app.private_schema_compatibility_journal_v50()
RETURNS TABLE(
...
REVOKE ALL ON FUNCTION app.private_schema_compatibility_journal_v50()
  FROM PUBLIC,periapsis_api,periapsis_worker,periapsis_notifier,
       periapsis_auditor;
--> statement-breakpoint

CREATE FUNCTION app.schema_compatibility_v50()
RETURNS TABLE(
...
BEGIN
  ...
  FROM app.private_schema_compatibility_journal_v50() AS journal;
  ...
  IF journal_count=232 AND journal_latest_created_at=1788650095675
...
END;
$function$;
ALTER FUNCTION app.schema_compatibility_v50() OWNER TO periapsis_migrator;
REVOKE ALL ON FUNCTION app.schema_compatibility_v50()
  FROM PUBLIC,periapsis_api,periapsis_worker,periapsis_notifier,
       periapsis_auditor;
GRANT EXECUTE ON FUNCTION app.schema_compatibility_v50()
...
CREATE FUNCTION app.private_release_runtime_dependency_surface_hash_v50()
...
self_excluded_function(function_oid) AS (
  VALUES
    (pg_catalog.to_regprocedure('app.schema_compatibility_v50()')),
    (pg_catalog.to_regprocedure('app.private_release_runtime_dependency_surface_hash_v50()')),
    (pg_catalog.to_regprocedure('app.private_release_runtime_schema_readiness_v50()')),
...
FROM catalog_entry;
$function$;
ALTER FUNCTION app.private_release_runtime_dependency_surface_hash_v50()
  OWNER TO periapsis_migrator;
...
CREATE FUNCTION app.private_release_runtime_schema_readiness_v50()
...
BEGIN
  IF current_setting('server_version_num')::integer<180000
     ...
     OR app.private_release_runtime_dependency_surface_hash_v50()<>
        'e68c7797c4f72188d1ddd4d133e5adff3f099004578fa77ad9f6472027fea9f8' THEN
    RETURN false;
  END IF;
...
END;
$function$;
ALTER FUNCTION app.private_release_runtime_schema_readiness_v50()
  OWNER TO periapsis_migrator;
REVOKE ALL ON FUNCTION app.private_release_runtime_schema_readiness_v50()
  FROM PUBLIC,periapsis_api,periapsis_worker,periapsis_notifier,
       periapsis_auditor;
--> statement-breakpoint

CREATE FUNCTION app.release_runtime_schema_readiness_v50()
RETURNS boolean LANGUAGE plpgsql STABLE SECURITY DEFINER
SET search_path=pg_catalog,public,app AS $function$
DECLARE current_count bigint;
BEGIN
  SELECT compatibility.applied_count INTO current_count
  FROM app.schema_compatibility_v50() AS compatibility;
  RETURN current_count=232
    AND app.private_release_runtime_schema_readiness_v50()
    AND (
...
END;
$function$;
ALTER FUNCTION app.release_runtime_schema_readiness_v50()
  OWNER TO periapsis_migrator;
REVOKE ALL ON FUNCTION app.release_runtime_schema_readiness_v50()
  FROM PUBLIC,periapsis_api,periapsis_worker,periapsis_notifier,
       periapsis_auditor;
GRANT EXECUTE ON FUNCTION app.release_runtime_schema_readiness_v50()
  TO periapsis_api,periapsis_worker,
     periapsis_notification_readiness_owner;
--> statement-breakpoint

-- Feature-specific production probes all require the sealed V50 release root.
...
CREATE FUNCTION app.federated_authentication_schema_readiness_v50()
RETURNS boolean LANGUAGE plpgsql STABLE SECURITY DEFINER
SET search_path=pg_catalog,public,app AS $function$
BEGIN RETURN app.release_runtime_schema_readiness_v50();
EXCEPTION WHEN undefined_table OR undefined_function OR insufficient_privilege
  OR invalid_schema_name OR cardinality_violation OR data_exception THEN
  RETURN false;
END;
$function$;
CREATE FUNCTION app.platform_oidc_direct_runtime_schema_readiness_v50()
RETURNS boolean LANGUAGE plpgsql STABLE SECURITY DEFINER
SET search_path=pg_catalog,public,app AS $function$
BEGIN RETURN app.release_runtime_schema_readiness_v50();
EXCEPTION WHEN undefined_table OR undefined_function OR insufficient_privilege
...
END;
$function$;
CREATE FUNCTION app.platform_saml_direct_runtime_schema_readiness_v50()
RETURNS boolean LANGUAGE plpgsql STABLE SECURITY DEFINER
SET search_path=pg_catalog,public,app AS $function$
BEGIN RETURN app.release_runtime_schema_readiness_v50();
EXCEPTION WHEN undefined_table OR undefined_function OR insufficient_privilege
...
END;
$function$;
CREATE FUNCTION app.platform_local_account_runtime_schema_readiness_v50()
RETURNS boolean LANGUAGE plpgsql STABLE SECURITY DEFINER
SET search_path=pg_catalog,public,app AS $function$
BEGIN RETURN app.release_runtime_schema_readiness_v50()
...
END;
$function$;
CREATE FUNCTION app.sla_trigger_action_runtime_schema_readiness_v50()
RETURNS boolean LANGUAGE plpgsql STABLE SECURITY DEFINER
SET search_path=pg_catalog,public,app AS $function$
BEGIN RETURN app.release_runtime_schema_readiness_v50()
...
END;
$function$;
CREATE FUNCTION app.sla_object_event_ingress_schema_readiness_v50()
RETURNS boolean LANGUAGE plpgsql STABLE SECURITY DEFINER
SET search_path=pg_catalog,public,app AS $function$
BEGIN RETURN app.release_runtime_schema_readiness_v50()
...
END;
$function$;
CREATE FUNCTION app.ticket_bulk_runtime_schema_readiness_v50()
RETURNS boolean LANGUAGE plpgsql STABLE SECURITY DEFINER
SET search_path=pg_catalog,public,app AS $function$
BEGIN RETURN app.release_runtime_schema_readiness_v50()
...
END;
$function$;
CREATE FUNCTION app.ticket_export_runtime_schema_readiness_v50()
RETURNS boolean LANGUAGE plpgsql STABLE SECURITY DEFINER
SET search_path=pg_catalog,public,app AS $function$
BEGIN RETURN app.release_runtime_schema_readiness_v50()
...
END;
$function$;
CREATE FUNCTION app.ticket_metadata_runtime_schema_readiness_v50()
RETURNS boolean LANGUAGE plpgsql STABLE SECURITY DEFINER
SET search_path=pg_catalog,public,app AS $function$
BEGIN RETURN app.release_runtime_schema_readiness_v50()
...
CREATE FUNCTION app.notification_dispatch_readiness_v50()
RETURNS TABLE(
  queue_depth bigint,
  role_safe boolean,
  schema_safe boolean,
...
ALTER FUNCTION app.federated_authentication_schema_readiness_v50()
  OWNER TO periapsis_migrator;
ALTER FUNCTION app.platform_oidc_direct_runtime_schema_readiness_v50()
  OWNER TO periapsis_migrator;
ALTER FUNCTION app.platform_saml_direct_runtime_schema_readiness_v50()
  OWNER TO periapsis_migrator;
ALTER FUNCTION app.platform_local_account_runtime_schema_readiness_v50()
  OWNER TO periapsis_migrator;
ALTER FUNCTION app.sla_trigger_action_runtime_schema_readiness_v50()
  OWNER TO periapsis_migrator;
ALTER FUNCTION app.sla_object_event_ingress_schema_readiness_v50()
  OWNER TO periapsis_migrator;
ALTER FUNCTION app.ticket_bulk_runtime_schema_readiness_v50()
  OWNER TO periapsis_migrator;
ALTER FUNCTION app.ticket_export_runtime_schema_readiness_v50()
  OWNER TO periapsis_migrator;
ALTER FUNCTION app.ticket_metadata_runtime_schema_readiness_v50()
  OWNER TO periapsis_migrator;
ALTER FUNCTION app.notification_dispatch_readiness_v50()
...
CREATE FUNCTION app.private_rotate_notification_readiness_v50()
...
END;
$function$;
ALTER FUNCTION app.private_rotate_notification_readiness_v50()
  OWNER TO periapsis_notification_readiness_owner;
REVOKE ALL ON FUNCTION app.private_rotate_notification_readiness_v50()
  FROM PUBLIC,periapsis_api,periapsis_worker,periapsis_notifier,
       periapsis_auditor,periapsis_notification_admin_owner,
       periapsis_notification_dispatch_owner;
GRANT EXECUTE ON FUNCTION app.private_rotate_notification_readiness_v50()
...
DECLARE
  ...
  was_sealed boolean;
BEGIN
  IF p_expected_count IS DISTINCT FROM 232
     OR p_expected_latest_created_at IS DISTINCT FROM 1788650095675
     OR p_expected_latest_hash !~ '^[0-9a-f]{64}$'
     OR p_expected_migration_fingerprint !~
       '^[0-9]+@[0-9a-f]{64}(:[0-9]+@[0-9a-f]{64}){231}$' THEN
    RAISE EXCEPTION 'schema compatibility v50 seal input is invalid'
      USING ERRCODE='55000';
  END IF;
  ...
  IF ROW(journal_count,journal_latest_created_at,journal_latest_hash,
     ...
     OR NOT app.private_release_runtime_schema_readiness_v50() THEN
    RAISE EXCEPTION 'schema compatibility v50 pre-seal verification failed'
      USING ERRCODE='55000';
  END IF;

  SELECT coalesce((
    SELECT compatibility.applied_count=232
    FROM app.schema_compatibility_v50() AS compatibility
  ),false) INTO was_sealed;

  IF NOT was_sealed THEN
    EXECUTE pg_catalog.format(
      'ALTER FUNCTION app.schema_compatibility_v50() SET app.schema_compatibility_fingerprint = %L',
      p_expected_migration_fingerprint
    );
    ...
    PERFORM app.private_rotate_notification_readiness_v50();
  ...
  IF NOT was_sealed THEN
    IF sealed_count<>232
       OR NOT app.private_release_runtime_schema_readiness_v50() THEN
      RAISE EXCEPTION 'schema compatibility v50 rotation verification failed'
        USING ERRCODE='55000';
    END IF;
    RETURN;
  END IF;
  IF sealed_count<>232
     OR NOT app.private_release_runtime_schema_readiness_v50()
     OR NOT app.release_runtime_schema_readiness_v50()
     OR NOT app.federated_authentication_schema_readiness_v50()
     OR NOT app.platform_oidc_direct_runtime_schema_readiness_v50()
     OR NOT app.platform_saml_direct_runtime_schema_readiness_v50()
     OR NOT app.platform_local_account_runtime_schema_readiness_v50()
     OR NOT app.sla_trigger_action_runtime_schema_readiness_v50()
     OR NOT app.sla_object_event_ingress_schema_readiness_v50()
     OR NOT app.ticket_bulk_runtime_schema_readiness_v50()
     OR NOT app.ticket_export_runtime_schema_readiness_v50()
     OR NOT app.ticket_metadata_runtime_schema_readiness_v50()
     OR NOT (SELECT readiness.schema_safe
             FROM app.notification_dispatch_readiness_v50() AS readiness) THEN
    RAISE EXCEPTION 'schema compatibility v50 post-seal verification failed'
      USING ERRCODE='55000';
  END IF;
END;
$function$;
