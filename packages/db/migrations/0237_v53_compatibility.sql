-- V53 seals the complete 0000-0237 prefix, including tenant LDAP configuration.
-- The catalog digest was derived from these routines on fresh PostgreSQL 18.6
-- with UTF8/C; only the source-attested migration runner may seal the release.
-- All historical function sources remain pinned, including the V52 aggregates.
CREATE FUNCTION app.private_schema_compatibility_journal_v53()
RETURNS TABLE(
...
REVOKE ALL ON FUNCTION app.private_schema_compatibility_journal_v53()
  FROM PUBLIC,periapsis_api,periapsis_worker,periapsis_notifier,
       periapsis_auditor;
--> statement-breakpoint

CREATE FUNCTION app.schema_compatibility_v53()
RETURNS TABLE(
...
BEGIN
  ...
  FROM app.private_schema_compatibility_journal_v53() AS journal;
  ...
  IF journal_count=238 AND journal_latest_created_at=1788790964334
...
END;
$function$;
ALTER FUNCTION app.schema_compatibility_v53() OWNER TO periapsis_migrator;
REVOKE ALL ON FUNCTION app.schema_compatibility_v53()
  FROM PUBLIC,periapsis_api,periapsis_worker,periapsis_notifier,
       periapsis_auditor;
GRANT EXECUTE ON FUNCTION app.schema_compatibility_v53()
...
CREATE FUNCTION app.private_release_runtime_dependency_surface_hash_v53()
...
self_excluded_function(function_oid) AS (
  VALUES
    (pg_catalog.to_regprocedure('app.schema_compatibility_v53()')),
    (pg_catalog.to_regprocedure('app.private_release_runtime_dependency_surface_hash_v53()')),
    (pg_catalog.to_regprocedure('app.private_release_runtime_schema_readiness_v53()')),
...
FROM catalog_entry;
$function$;
ALTER FUNCTION app.private_release_runtime_dependency_surface_hash_v53()
  OWNER TO periapsis_migrator;
...
CREATE FUNCTION app.private_release_runtime_schema_readiness_v53()
...
BEGIN
  IF current_setting('server_version_num')::integer<180000
     ...
     OR app.private_release_runtime_dependency_surface_hash_v53()<>
    '25836ab746715d3cec731946d6c5730998b42ca4cf48185346c5254823ef0b51' THEN
    RETURN false;
  END IF;
...
END;
$function$;
ALTER FUNCTION app.private_release_runtime_schema_readiness_v53()
  OWNER TO periapsis_migrator;
REVOKE ALL ON FUNCTION app.private_release_runtime_schema_readiness_v53()
  FROM PUBLIC,periapsis_api,periapsis_worker,periapsis_notifier,
       periapsis_auditor;
--> statement-breakpoint

CREATE FUNCTION app.release_runtime_schema_readiness_v53()
RETURNS boolean LANGUAGE plpgsql STABLE SECURITY DEFINER
SET search_path=pg_catalog,public,app AS $function$
DECLARE current_count bigint;
BEGIN
  SELECT compatibility.applied_count INTO current_count
  FROM app.schema_compatibility_v53() AS compatibility;
  RETURN current_count=238
    AND app.private_release_runtime_schema_readiness_v53()
    AND (
      ...
      SELECT count(*)=9 AND coalesce(bool_and(
...
END;
$function$;
ALTER FUNCTION app.release_runtime_schema_readiness_v53()
  OWNER TO periapsis_migrator;
REVOKE ALL ON FUNCTION app.release_runtime_schema_readiness_v53()
  FROM PUBLIC,periapsis_api,periapsis_worker,periapsis_notifier,
       periapsis_auditor;
GRANT EXECUTE ON FUNCTION app.release_runtime_schema_readiness_v53()
  TO periapsis_api,periapsis_worker,
     periapsis_notification_readiness_owner;
--> statement-breakpoint

-- Feature-specific production probes all require the sealed V53 release root.
...
CREATE FUNCTION app.federated_authentication_schema_readiness_v53()
RETURNS boolean LANGUAGE plpgsql STABLE SECURITY DEFINER
SET search_path=pg_catalog,public,app AS $function$
BEGIN RETURN app.release_runtime_schema_readiness_v53();
EXCEPTION WHEN undefined_table OR undefined_function OR insufficient_privilege
  OR invalid_schema_name OR cardinality_violation OR data_exception THEN
  RETURN false;
END;
$function$;
CREATE FUNCTION app.platform_oidc_direct_runtime_schema_readiness_v53()
RETURNS boolean LANGUAGE plpgsql STABLE SECURITY DEFINER
SET search_path=pg_catalog,public,app AS $function$
BEGIN RETURN app.release_runtime_schema_readiness_v53();
EXCEPTION WHEN undefined_table OR undefined_function OR insufficient_privilege
...
END;
$function$;
CREATE FUNCTION app.platform_saml_direct_runtime_schema_readiness_v53()
RETURNS boolean LANGUAGE plpgsql STABLE SECURITY DEFINER
SET search_path=pg_catalog,public,app AS $function$
BEGIN RETURN app.release_runtime_schema_readiness_v53();
EXCEPTION WHEN undefined_table OR undefined_function OR insufficient_privilege
...
END;
$function$;
CREATE FUNCTION app.platform_local_account_runtime_schema_readiness_v53()
RETURNS boolean LANGUAGE plpgsql STABLE SECURITY DEFINER
SET search_path=pg_catalog,public,app AS $function$
BEGIN RETURN app.release_runtime_schema_readiness_v53()
...
END;
$function$;
CREATE FUNCTION app.sla_trigger_action_runtime_schema_readiness_v53()
RETURNS boolean LANGUAGE plpgsql STABLE SECURITY DEFINER
SET search_path=pg_catalog,public,app AS $function$
BEGIN RETURN app.release_runtime_schema_readiness_v53()
...
END;
$function$;
CREATE FUNCTION app.sla_object_event_ingress_schema_readiness_v53()
RETURNS boolean LANGUAGE plpgsql STABLE SECURITY DEFINER
SET search_path=pg_catalog,public,app AS $function$
BEGIN RETURN app.release_runtime_schema_readiness_v53()
...
END;
$function$;
CREATE FUNCTION app.ticket_bulk_runtime_schema_readiness_v53()
RETURNS boolean LANGUAGE plpgsql STABLE SECURITY DEFINER
SET search_path=pg_catalog,public,app AS $function$
BEGIN RETURN app.release_runtime_schema_readiness_v53()
...
END;
$function$;
CREATE FUNCTION app.ticket_export_runtime_schema_readiness_v53()
RETURNS boolean LANGUAGE plpgsql STABLE SECURITY DEFINER
SET search_path=pg_catalog,public,app AS $function$
BEGIN RETURN app.release_runtime_schema_readiness_v53()
...
END;
$function$;
CREATE FUNCTION app.ticket_metadata_runtime_schema_readiness_v53()
RETURNS boolean LANGUAGE plpgsql STABLE SECURITY DEFINER
SET search_path=pg_catalog,public,app AS $function$
BEGIN RETURN app.release_runtime_schema_readiness_v53()
...
CREATE FUNCTION app.notification_dispatch_readiness_v53()
RETURNS TABLE(
  queue_depth bigint,
  role_safe boolean,
  schema_safe boolean,
...
BEGIN
  ...
  schema_safe := app.release_runtime_schema_readiness_v53();
  RETURN NEXT;
EXCEPTION WHEN undefined_table OR undefined_function OR insufficient_privilege
  OR invalid_schema_name OR cardinality_violation OR data_exception THEN
  queue_depth := 0;
...
ALTER FUNCTION app.federated_authentication_schema_readiness_v53()
  OWNER TO periapsis_migrator;
ALTER FUNCTION app.platform_oidc_direct_runtime_schema_readiness_v53()
  OWNER TO periapsis_migrator;
ALTER FUNCTION app.platform_saml_direct_runtime_schema_readiness_v53()
  OWNER TO periapsis_migrator;
ALTER FUNCTION app.platform_local_account_runtime_schema_readiness_v53()
  OWNER TO periapsis_migrator;
ALTER FUNCTION app.sla_trigger_action_runtime_schema_readiness_v53()
  OWNER TO periapsis_migrator;
ALTER FUNCTION app.sla_object_event_ingress_schema_readiness_v53()
  OWNER TO periapsis_migrator;
ALTER FUNCTION app.ticket_bulk_runtime_schema_readiness_v53()
  OWNER TO periapsis_migrator;
ALTER FUNCTION app.ticket_export_runtime_schema_readiness_v53()
  OWNER TO periapsis_migrator;
ALTER FUNCTION app.ticket_metadata_runtime_schema_readiness_v53()
  OWNER TO periapsis_migrator;
ALTER FUNCTION app.notification_dispatch_readiness_v53()
...
CREATE FUNCTION app.private_rotate_notification_readiness_v53()
...
END;
$function$;
ALTER FUNCTION app.private_rotate_notification_readiness_v53()
  OWNER TO periapsis_notification_readiness_owner;
REVOKE ALL ON FUNCTION app.private_rotate_notification_readiness_v53()
  FROM PUBLIC,periapsis_api,periapsis_worker,periapsis_notifier,
       periapsis_auditor,periapsis_notification_admin_owner,
       periapsis_notification_dispatch_owner;
GRANT EXECUTE ON FUNCTION app.private_rotate_notification_readiness_v53()
...
DECLARE
  ...
  was_sealed boolean;
BEGIN
  IF p_expected_count IS DISTINCT FROM 238
     OR p_expected_latest_created_at IS DISTINCT FROM 1788790964334
     OR p_expected_latest_hash !~ '^[0-9a-f]{64}$'
     OR p_expected_migration_fingerprint !~
       '^[0-9]+@[0-9a-f]{64}(:[0-9]+@[0-9a-f]{64}){237}$' THEN
    RAISE EXCEPTION 'schema compatibility v53 seal input is invalid'
      USING ERRCODE='55000';
  END IF;
  ...
  IF ROW(journal_count,journal_latest_created_at,journal_latest_hash,
     ...
     OR NOT app.private_release_runtime_schema_readiness_v53() THEN
    RAISE EXCEPTION 'schema compatibility v53 pre-seal verification failed'
      USING ERRCODE='55000';
  END IF;

  SELECT coalesce((
    SELECT compatibility.applied_count=238
    FROM app.schema_compatibility_v53() AS compatibility
  ),false) INTO was_sealed;

  IF NOT was_sealed THEN
    EXECUTE pg_catalog.format(
      'ALTER FUNCTION app.schema_compatibility_v53() SET app.schema_compatibility_fingerprint = %L',
      p_expected_migration_fingerprint
    );
    ...
    PERFORM app.private_rotate_notification_readiness_v53();
  ...
  IF NOT was_sealed THEN
    IF sealed_count<>238
       OR NOT app.private_release_runtime_schema_readiness_v53() THEN
      RAISE EXCEPTION 'schema compatibility v53 rotation verification failed'
        USING ERRCODE='55000';
    END IF;
    RETURN;
  END IF;
  IF sealed_count<>238
     OR NOT app.private_release_runtime_schema_readiness_v53()
     OR NOT app.release_runtime_schema_readiness_v53()
     OR NOT app.federated_authentication_schema_readiness_v53()
     OR NOT app.platform_oidc_direct_runtime_schema_readiness_v53()
     OR NOT app.platform_saml_direct_runtime_schema_readiness_v53()
     OR NOT app.platform_local_account_runtime_schema_readiness_v53()
     OR NOT app.sla_trigger_action_runtime_schema_readiness_v53()
     OR NOT app.sla_object_event_ingress_schema_readiness_v53()
     OR NOT app.ticket_bulk_runtime_schema_readiness_v53()
     OR NOT app.ticket_export_runtime_schema_readiness_v53()
     OR NOT app.ticket_metadata_runtime_schema_readiness_v53()
     OR NOT (SELECT readiness.schema_safe
             FROM app.notification_dispatch_readiness_v53() AS readiness) THEN
    RAISE EXCEPTION 'schema compatibility v53 post-seal verification failed'
      USING ERRCODE='55000';
  END IF;
END;
$function$;
