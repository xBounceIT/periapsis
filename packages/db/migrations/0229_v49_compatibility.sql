-- V49 is the single runtime/release root for the complete 0000-0229
-- inventory. It preserves the immutable V47/V48 evidence while rotating
-- every current runtime compatibility edge to this release.
CREATE FUNCTION app.private_schema_compatibility_journal_v49()
RETURNS TABLE(
...
REVOKE ALL ON FUNCTION app.private_schema_compatibility_journal_v49()
  FROM PUBLIC,periapsis_api,periapsis_worker,periapsis_notifier,
       periapsis_auditor;
--> statement-breakpoint

CREATE FUNCTION app.schema_compatibility_v49()
RETURNS TABLE(
...
BEGIN
  ...
  FROM app.private_schema_compatibility_journal_v49() AS journal;
  ...
  IF journal_count=230 AND journal_latest_created_at=1788520217531
...
END;
$function$;
ALTER FUNCTION app.schema_compatibility_v49() OWNER TO periapsis_migrator;
REVOKE ALL ON FUNCTION app.schema_compatibility_v49()
  FROM PUBLIC,periapsis_api,periapsis_worker,periapsis_notifier,
       periapsis_auditor;
GRANT EXECUTE ON FUNCTION app.schema_compatibility_v49()
  TO periapsis_api,periapsis_worker;
--> statement-breakpoint

-- pg_roles intentionally masks every password as ********, including NULL.
-- A tightly bounded sealer-owned helper is therefore the only catalog path
-- that can distinguish an unusable LOGIN role without exposing a verifier.
-- It accepts no input and returns only a three-state classification for the
-- four fixed runtime logins.  Its source, owner class, configuration and ACL
-- are themselves part of the V49 dependency transcript.
DO $assert_v49_sealer_superuser$
BEGIN
  IF NOT EXISTS (
    SELECT 1
    ...
    RAISE EXCEPTION
      'V49 credential-state attestation requires a superuser sealer'
      USING ERRCODE='42501';
  END IF;
END;
$assert_v49_sealer_superuser$;
--> statement-breakpoint

CREATE FUNCTION app.private_runtime_login_credential_state_v49()
RETURNS TABLE(role_name text,credential_state text)
LANGUAGE sql STABLE SECURITY DEFINER
SET search_path=pg_catalog AS $function$
  WITH expected(role_name) AS (
    VALUES ('periapsis_api_login'),('periapsis_worker_login'),
           ('periapsis_notifier_login'),('periapsis_auditor_login')
  )
  SELECT expected.role_name,
    CASE
      WHEN credential.rolname IS NULL OR credential.rolpassword IS NULL
        THEN 'absent'
      WHEN credential.rolpassword LIKE 'SCRAM-SHA-256$%' THEN 'scram'
      ELSE 'other'
    END
  FROM expected
  LEFT JOIN pg_catalog.pg_authid AS credential
    ON credential.rolname=expected.role_name
  ORDER BY expected.role_name COLLATE "C"
$function$;
REVOKE ALL ON FUNCTION app.private_runtime_login_credential_state_v49()
  FROM PUBLIC,periapsis_api,periapsis_worker,periapsis_notifier,
       periapsis_auditor,periapsis_notification_readiness_owner,
       periapsis_custom_field_import_owner,periapsis_webhook_url_policy_owner,
       periapsis_migrator;
GRANT EXECUTE ON FUNCTION app.private_runtime_login_credential_state_v49()
...
-- The federated JIT writer predates PostgreSQL's core cryptographic
-- primitives and still called pgcrypto's gen_random_bytes even though the
-- extension is intentionally absent from the production schema.  Repair the
-- one sealed predecessor source in place.  Both the complete predecessor hash
-- and the single replacement coordinate are required so an unexpected source
-- can never be rewritten opportunistically.
DO $repair_federated_jit_subject_handle_v49$
DECLARE
  ...
  old_expression constant text := 'gen_random_bytes(32)';
...
BEGIN
  SELECT pg_catalog.pg_get_functiondef(function_row.oid),
    ...
    INTO STRICT predecessor_definition,predecessor_source,
  ...
  WHERE function_row.oid=function_oid
    ...
    AND pg_catalog.pg_get_function_result(function_row.oid)=
      'TABLE(out_user_id uuid, out_membership_id uuid, out_external_identity_id uuid, out_external_identity_revision bigint, out_identity_epoch bigint, out_session_invalidation_epoch bigint)'
    AND (
  ...
  IF predecessor_source_hash<>
     ...
     OR pg_catalog.length(predecessor_source)-pg_catalog.length(
       pg_catalog.replace(predecessor_source,old_expression,'')
     )<>pg_catalog.length(old_expression) THEN
    RAISE EXCEPTION
      'federated JIT identity writer drifted from its sealed predecessor'
      USING ERRCODE='55000';
  END IF;
  ...
  IF repaired_definition IS NOT DISTINCT FROM predecessor_definition
     OR pg_catalog.strpos(repaired_definition,old_expression)<>0
     OR pg_catalog.strpos(repaired_definition,new_expression)=0 THEN
    RAISE EXCEPTION 'federated JIT subject-handle repair is ambiguous'
      USING ERRCODE='55000';
  END IF;
  ...
  SELECT pg_catalog.encode(pg_catalog.sha256(pg_catalog.convert_to(
    ...
    INTO STRICT repaired_source_hash
  FROM pg_catalog.pg_proc AS function_row
  WHERE function_row.oid=function_oid
    AND pg_catalog.strpos(function_row.prosrc,old_expression)=0
    AND pg_catalog.strpos(function_row.prosrc,new_expression)>0;
  IF repaired_source_hash<>
       'f064d2874523976d14529a4ac239f4a6c892b6e50b212c6b126c90b8a5d2d022' THEN
    RAISE EXCEPTION 'federated JIT subject-handle repair is not exact'
      USING ERRCODE='55000';
  END IF;
END;
$repair_federated_jit_subject_handle_v49$;
--> statement-breakpoint
...
-- excluded; ownership, ACLs, RLS, columns, types, inheritance, casts,
-- constraints, indexes, policies, triggers, rewrite and publication surfaces,
-- role topology, session safety settings and every non-self-referential
-- public/app function source are canonicalized. Historical sealers deliberately changed
-- 46 exact routine ACLs, five exact compatibility configs and four exact
-- derived private sources; only source-attested states at those signatures
-- are normalized so fresh and rolling installs converge. Deployment logins
-- are omitted only while absent or in one of their exact lifecycle states;
-- every drifted attribute, setting, membership or direct object grant remains
-- covered.
-- The sealer and public root assert the resulting runtime rotation exactly.
CREATE FUNCTION app.private_release_runtime_dependency_surface_hash_v49()
...
self_excluded_function(function_oid) AS (
  VALUES
    (pg_catalog.to_regprocedure('app.schema_compatibility_v49()')),
    (pg_catalog.to_regprocedure('app.private_release_runtime_dependency_surface_hash_v49()')),
    (pg_catalog.to_regprocedure('app.private_release_runtime_schema_readiness_v49()')),
...
FROM catalog_entry;
$function$;
ALTER FUNCTION app.private_release_runtime_dependency_surface_hash_v49()
  OWNER TO periapsis_migrator;
...
CREATE FUNCTION app.private_release_runtime_schema_readiness_v49()
...
BEGIN
  IF current_setting('server_version_num')::integer<180000
     ...
     OR app.private_release_runtime_dependency_surface_hash_v49()<>
        '048078bb2a5c69057ec356857e323d55d0b97a43a5894e620140eab1f758414e' THEN
    RETURN false;
  END IF;
...
END;
$function$;
ALTER FUNCTION app.private_release_runtime_schema_readiness_v49()
  OWNER TO periapsis_migrator;
REVOKE ALL ON FUNCTION app.private_release_runtime_schema_readiness_v49()
  FROM PUBLIC,periapsis_api,periapsis_worker,periapsis_notifier,
       periapsis_auditor;
--> statement-breakpoint

CREATE FUNCTION app.release_runtime_schema_readiness_v49()
RETURNS boolean LANGUAGE plpgsql STABLE SECURITY DEFINER
SET search_path=pg_catalog,public,app AS $function$
...
BEGIN
  ...
  RETURN current_count=230
    AND app.private_release_runtime_schema_readiness_v49()
    AND (
...
END;
$function$;
ALTER FUNCTION app.release_runtime_schema_readiness_v49()
  OWNER TO periapsis_migrator;
REVOKE ALL ON FUNCTION app.release_runtime_schema_readiness_v49()
  FROM PUBLIC,periapsis_api,periapsis_worker,periapsis_notifier,
       periapsis_auditor;
GRANT EXECUTE ON FUNCTION app.release_runtime_schema_readiness_v49()
  TO periapsis_api,periapsis_worker,
     periapsis_notification_readiness_owner;
--> statement-breakpoint

-- Feature-specific production probes all require the sealed V49 release root.
...
CREATE FUNCTION app.federated_authentication_schema_readiness_v49()
RETURNS boolean LANGUAGE plpgsql STABLE SECURITY DEFINER
SET search_path=pg_catalog,public,app AS $function$
BEGIN RETURN app.release_runtime_schema_readiness_v49();
EXCEPTION WHEN undefined_table OR undefined_function OR insufficient_privilege
  OR invalid_schema_name OR cardinality_violation OR data_exception THEN
  RETURN false;
END;
$function$;
CREATE FUNCTION app.platform_oidc_direct_runtime_schema_readiness_v49()
RETURNS boolean LANGUAGE plpgsql STABLE SECURITY DEFINER
SET search_path=pg_catalog,public,app AS $function$
BEGIN RETURN app.release_runtime_schema_readiness_v49();
EXCEPTION WHEN undefined_table OR undefined_function OR insufficient_privilege
...
END;
$function$;
CREATE FUNCTION app.platform_saml_direct_runtime_schema_readiness_v49()
RETURNS boolean LANGUAGE plpgsql STABLE SECURITY DEFINER
SET search_path=pg_catalog,public,app AS $function$
BEGIN RETURN app.release_runtime_schema_readiness_v49();
EXCEPTION WHEN undefined_table OR undefined_function OR insufficient_privilege
...
END;
$function$;
CREATE FUNCTION app.platform_local_account_runtime_schema_readiness_v49()
RETURNS boolean LANGUAGE plpgsql STABLE SECURITY DEFINER
SET search_path=pg_catalog,public,app AS $function$
BEGIN RETURN app.release_runtime_schema_readiness_v49()
...
END;
$function$;
CREATE FUNCTION app.sla_trigger_action_runtime_schema_readiness_v49()
RETURNS boolean LANGUAGE plpgsql STABLE SECURITY DEFINER
SET search_path=pg_catalog,public,app AS $function$
BEGIN RETURN app.release_runtime_schema_readiness_v49()
...
END;
$function$;
CREATE FUNCTION app.sla_object_event_ingress_schema_readiness_v49()
RETURNS boolean LANGUAGE plpgsql STABLE SECURITY DEFINER
SET search_path=pg_catalog,public,app AS $function$
BEGIN RETURN app.release_runtime_schema_readiness_v49()
...
END;
$function$;
CREATE FUNCTION app.ticket_bulk_runtime_schema_readiness_v49()
RETURNS boolean LANGUAGE plpgsql STABLE SECURITY DEFINER
SET search_path=pg_catalog,public,app AS $function$
BEGIN RETURN app.release_runtime_schema_readiness_v49()
...
END;
$function$;
CREATE FUNCTION app.ticket_export_runtime_schema_readiness_v49()
RETURNS boolean LANGUAGE plpgsql STABLE SECURITY DEFINER
SET search_path=pg_catalog,public,app AS $function$
BEGIN RETURN app.release_runtime_schema_readiness_v49()
...
END;
$function$;
CREATE FUNCTION app.ticket_metadata_runtime_schema_readiness_v49()
RETURNS boolean LANGUAGE plpgsql STABLE SECURITY DEFINER
SET search_path=pg_catalog,public,app AS $function$
BEGIN RETURN app.release_runtime_schema_readiness_v49()
...
CREATE FUNCTION app.notification_dispatch_readiness_v49()
RETURNS TABLE(
  queue_depth bigint,
  role_safe boolean,
  schema_safe boolean,
...
ALTER FUNCTION app.federated_authentication_schema_readiness_v49()
  OWNER TO periapsis_migrator;
ALTER FUNCTION app.platform_oidc_direct_runtime_schema_readiness_v49()
  OWNER TO periapsis_migrator;
ALTER FUNCTION app.platform_saml_direct_runtime_schema_readiness_v49()
  OWNER TO periapsis_migrator;
ALTER FUNCTION app.platform_local_account_runtime_schema_readiness_v49()
  OWNER TO periapsis_migrator;
ALTER FUNCTION app.sla_trigger_action_runtime_schema_readiness_v49()
  OWNER TO periapsis_migrator;
ALTER FUNCTION app.sla_object_event_ingress_schema_readiness_v49()
  OWNER TO periapsis_migrator;
ALTER FUNCTION app.ticket_bulk_runtime_schema_readiness_v49()
  OWNER TO periapsis_migrator;
ALTER FUNCTION app.ticket_export_runtime_schema_readiness_v49()
  OWNER TO periapsis_migrator;
ALTER FUNCTION app.ticket_metadata_runtime_schema_readiness_v49()
  OWNER TO periapsis_migrator;
ALTER FUNCTION app.notification_dispatch_readiness_v49()
...
CREATE FUNCTION app.private_rotate_notification_readiness_v49()
...
END;
$function$;
ALTER FUNCTION app.private_rotate_notification_readiness_v49()
  OWNER TO periapsis_notification_readiness_owner;
REVOKE ALL ON FUNCTION app.private_rotate_notification_readiness_v49()
  FROM PUBLIC,periapsis_api,periapsis_worker,periapsis_notifier,
       periapsis_auditor,periapsis_notification_admin_owner,
       periapsis_notification_dispatch_owner;
GRANT EXECUTE ON FUNCTION app.private_rotate_notification_readiness_v49()
  TO periapsis_migrator;
--> statement-breakpoint
...
DECLARE
  ...
  was_sealed boolean;
BEGIN
  IF p_expected_count IS DISTINCT FROM 230
     OR p_expected_latest_created_at IS DISTINCT FROM 1788520217531
     OR p_expected_latest_hash !~ '^[0-9a-f]{64}$'
     OR p_expected_migration_fingerprint !~
       '^[0-9]+@[0-9a-f]{64}(:[0-9]+@[0-9a-f]{64}){229}$' THEN
    RAISE EXCEPTION 'schema compatibility v49 seal input is invalid'
      USING ERRCODE='55000';
  END IF;
  ...
  IF ROW(journal_count,journal_latest_created_at,journal_latest_hash,
     ...
     OR NOT app.private_release_runtime_schema_readiness_v49() THEN
    RAISE EXCEPTION 'schema compatibility v49 pre-seal verification failed'
      USING ERRCODE='55000';
  END IF;

  SELECT coalesce((
    SELECT compatibility.applied_count=230
    FROM app.schema_compatibility_v49() AS compatibility
  ),false) INTO was_sealed;

  IF NOT was_sealed THEN
    EXECUTE pg_catalog.format(
      'ALTER FUNCTION app.schema_compatibility_v49() SET app.schema_compatibility_fingerprint = %L',
      p_expected_migration_fingerprint
    );
    ...
    PERFORM app.private_rotate_notification_readiness_v49();
  ...
  IF NOT was_sealed THEN
    IF sealed_count<>230
       OR NOT app.private_release_runtime_schema_readiness_v49() THEN
      RAISE EXCEPTION 'schema compatibility v49 rotation verification failed'
        USING ERRCODE='55000';
    END IF;
    RETURN;
  END IF;
  IF sealed_count<>230
     OR NOT app.private_release_runtime_schema_readiness_v49()
     OR NOT app.release_runtime_schema_readiness_v49()
     OR NOT app.federated_authentication_schema_readiness_v49()
     OR NOT app.platform_oidc_direct_runtime_schema_readiness_v49()
     OR NOT app.platform_saml_direct_runtime_schema_readiness_v49()
     OR NOT app.platform_local_account_runtime_schema_readiness_v49()
     OR NOT app.sla_trigger_action_runtime_schema_readiness_v49()
     OR NOT app.sla_object_event_ingress_schema_readiness_v49()
     OR NOT app.ticket_bulk_runtime_schema_readiness_v49()
     OR NOT app.ticket_export_runtime_schema_readiness_v49()
     OR NOT app.ticket_metadata_runtime_schema_readiness_v49()
     OR NOT (SELECT readiness.schema_safe
             FROM app.notification_dispatch_readiness_v49() AS readiness) THEN
    RAISE EXCEPTION 'schema compatibility v49 post-seal verification failed'
      USING ERRCODE='55000';
  END IF;
END;
$function$;
