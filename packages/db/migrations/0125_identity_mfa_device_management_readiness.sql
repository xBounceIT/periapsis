-- Device-management readiness advances the current compatibility projection
-- to v26. V25 is the sole exact rolling predecessor; v24 and older retire.

CREATE FUNCTION app.schema_compatibility_v26()
RETURNS TABLE(
...
DECLARE
  ...
  migration_0125_rows bigint;
BEGIN
  EXECUTE $query$
    SELECT count(*)::bigint, max(migration.created_at)::bigint,
           count(*) FILTER (
             WHERE migration.created_at = 1787723203494
           )::bigint
    FROM drizzle.__drizzle_migrations AS migration
  $query$ INTO journal_count, journal_latest_created_at, migration_0125_rows;
  IF journal_count = 126
     AND journal_latest_created_at = 1787723203494
     AND migration_0125_rows = 1 THEN
...
BEGIN
  ...
  IF full_count = 126
     AND full_latest_created_at = 1787723203494
...
BEGIN
  ...
  IF p_expected_count IS DISTINCT FROM 126
     OR p_expected_latest_created_at IS DISTINCT FROM 1787723203494
     ...
     OR cardinality(fingerprint_entries) IS DISTINCT FROM 126
     OR fingerprint_entries[126] IS DISTINCT FROM (
       p_expected_latest_created_at::text || '@' || p_expected_latest_hash
     ) THEN
    RAISE EXCEPTION 'invalid schema compatibility v26 manifest'
      USING ERRCODE = '22023';
  END IF;
  SELECT array_agg(
    split_part(entry.value, '@', 1)::bigint ORDER BY entry.ordinality
  ) INTO appended_created_at
  FROM unnest(fingerprint_entries[125:126]) WITH ORDINALITY
       AS entry(value, ordinality);
  IF appended_created_at IS DISTINCT FROM ARRAY[
    1787723201274, 1787723203494
  ]::bigint[] THEN
    RAISE EXCEPTION 'invalid schema compatibility v26 appended sequence'
      USING ERRCODE = '22023';
  END IF;
  ...
  IF actual_count IS DISTINCT FROM p_expected_count
    ...
    RAISE EXCEPTION 'schema compatibility v26 manifest does not match the applied journal'
      USING ERRCODE = '55000';
  END IF;
  ...
  IF predecessor_count IS DISTINCT FROM 124
     OR predecessor_latest_created_at IS DISTINCT FROM 1787721876116
    ...
    RAISE EXCEPTION 'sealed schema compatibility v25 prefix is not exact'
      USING ERRCODE = '55000';
  END IF;
  ...
  IF retired_count IS DISTINCT FROM 0
     OR has_function_privilege(
       'periapsis_api', 'app.schema_compatibility_v24()'::regprocedure,
       'EXECUTE'
     )
     OR has_function_privilege(
       'periapsis_worker', 'app.schema_compatibility_v24()'::regprocedure,
       'EXECUTE'
     ) THEN
    RAISE EXCEPTION 'legacy schema compatibility v24 remains active'
      USING ERRCODE = '55000';
  END IF;
END;
$function$;
...
CREATE FUNCTION app.identity_mfa_device_management_readiness_v1()
RETURNS boolean
LANGUAGE plpgsql
STABLE
SECURITY DEFINER
...
BEGIN
  ...
  SELECT compatibility.applied_count INTO retired_count
  FROM app.schema_compatibility_v24() AS compatibility;
  RETURN current_count = 126 AND predecessor_count = 124
    AND retired_count = 0;
END;
$function$;
