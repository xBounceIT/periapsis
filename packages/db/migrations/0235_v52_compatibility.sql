-- V52 seals the complete 0000-0235 prefix, including service readiness aggregates.
-- The catalog digest is derived on an owned PostgreSQL 18.6 UTF8/C cluster;
-- only the normal source-attested migration runner seals the release.
-- API/worker aggregates stay inside the full catalog transcript and perform
-- one release attestation per invocation, without caching across statements.
-- Historical sources remain pinned; twelve exact V51 ACL pairs and three
-- V51 compatibility config states extend the four exact current exclusions.
CREATE FUNCTION app.private_schema_compatibility_journal_v52()
RETURNS TABLE(
...
REVOKE ALL ON FUNCTION app.private_schema_compatibility_journal_v52()
  FROM PUBLIC,periapsis_api,periapsis_worker,periapsis_notifier,
       periapsis_auditor;
--> statement-breakpoint

CREATE FUNCTION app.schema_compatibility_v52()
RETURNS TABLE(
...
BEGIN
  ...
  FROM app.private_schema_compatibility_journal_v52() AS journal;
  ...
  IF journal_count=236 AND journal_latest_created_at=1788707601308
...
END;
$function$;
ALTER FUNCTION app.schema_compatibility_v52() OWNER TO periapsis_migrator;
REVOKE ALL ON FUNCTION app.schema_compatibility_v52()
  FROM PUBLIC,periapsis_api,periapsis_worker,periapsis_notifier,
       periapsis_auditor;
GRANT EXECUTE ON FUNCTION app.schema_compatibility_v52()
...
CREATE FUNCTION app.private_release_runtime_dependency_surface_hash_v52()
...
self_excluded_function(function_oid) AS (
  VALUES
    (pg_catalog.to_regprocedure('app.schema_compatibility_v52()')),
    (pg_catalog.to_regprocedure('app.private_release_runtime_dependency_surface_hash_v52()')),
    (pg_catalog.to_regprocedure('app.private_release_runtime_schema_readiness_v52()')),
...
FROM catalog_entry;
$function$;
ALTER FUNCTION app.private_release_runtime_dependency_surface_hash_v52()
  OWNER TO periapsis_migrator;
...
CREATE FUNCTION app.private_release_runtime_schema_readiness_v52()
...
BEGIN
  IF current_setting('server_version_num')::integer<180000
     ...
     OR app.private_release_runtime_dependency_surface_hash_v52()<>
    '7782b810b281fefee6142a0be6bd1691a7fc66f90be197322c95755a658e5b9f' THEN
    RETURN false;
  END IF;
...
END;
$function$;
ALTER FUNCTION app.private_release_runtime_schema_readiness_v52()
  OWNER TO periapsis_migrator;
REVOKE ALL ON FUNCTION app.private_release_runtime_schema_readiness_v52()
  FROM PUBLIC,periapsis_api,periapsis_worker,periapsis_notifier,
       periapsis_auditor;
--> statement-breakpoint

CREATE FUNCTION app.release_runtime_schema_readiness_v52()
RETURNS boolean LANGUAGE plpgsql STABLE SECURITY DEFINER
SET search_path=pg_catalog,public,app AS $function$
DECLARE current_count bigint;
BEGIN
  SELECT compatibility.applied_count INTO current_count
  FROM app.schema_compatibility_v52() AS compatibility;
  RETURN current_count=236
    AND app.private_release_runtime_schema_readiness_v52()
    AND (
...
END;
$function$;
ALTER FUNCTION app.release_runtime_schema_readiness_v52()
  OWNER TO periapsis_migrator;
REVOKE ALL ON FUNCTION app.release_runtime_schema_readiness_v52()
  FROM PUBLIC,periapsis_api,periapsis_worker,periapsis_notifier,
       periapsis_auditor;
GRANT EXECUTE ON FUNCTION app.release_runtime_schema_readiness_v52()
  TO periapsis_api,periapsis_worker,
     periapsis_notification_readiness_owner;
--> statement-breakpoint

-- Feature-specific production probes all require the sealed V52 release root.
...
CREATE FUNCTION app.federated_authentication_schema_readiness_v52()
RETURNS boolean LANGUAGE plpgsql STABLE SECURITY DEFINER
SET search_path=pg_catalog,public,app AS $function$
BEGIN RETURN app.release_runtime_schema_readiness_v52();
EXCEPTION WHEN undefined_table OR undefined_function OR insufficient_privilege
  OR invalid_schema_name OR cardinality_violation OR data_exception THEN
  RETURN false;
END;
$function$;
CREATE FUNCTION app.platform_oidc_direct_runtime_schema_readiness_v52()
RETURNS boolean LANGUAGE plpgsql STABLE SECURITY DEFINER
SET search_path=pg_catalog,public,app AS $function$
BEGIN RETURN app.release_runtime_schema_readiness_v52();
EXCEPTION WHEN undefined_table OR undefined_function OR insufficient_privilege
...
END;
$function$;
CREATE FUNCTION app.platform_saml_direct_runtime_schema_readiness_v52()
RETURNS boolean LANGUAGE plpgsql STABLE SECURITY DEFINER
SET search_path=pg_catalog,public,app AS $function$
BEGIN RETURN app.release_runtime_schema_readiness_v52();
EXCEPTION WHEN undefined_table OR undefined_function OR insufficient_privilege
...
END;
$function$;
CREATE FUNCTION app.platform_local_account_runtime_schema_readiness_v52()
RETURNS boolean LANGUAGE plpgsql STABLE SECURITY DEFINER
SET search_path=pg_catalog,public,app AS $function$
BEGIN RETURN app.release_runtime_schema_readiness_v52()
...
END;
$function$;
CREATE FUNCTION app.sla_trigger_action_runtime_schema_readiness_v52()
RETURNS boolean LANGUAGE plpgsql STABLE SECURITY DEFINER
SET search_path=pg_catalog,public,app AS $function$
BEGIN RETURN app.release_runtime_schema_readiness_v52()
...
END;
$function$;
CREATE FUNCTION app.sla_object_event_ingress_schema_readiness_v52()
RETURNS boolean LANGUAGE plpgsql STABLE SECURITY DEFINER
SET search_path=pg_catalog,public,app AS $function$
BEGIN RETURN app.release_runtime_schema_readiness_v52()
...
END;
$function$;
CREATE FUNCTION app.ticket_bulk_runtime_schema_readiness_v52()
RETURNS boolean LANGUAGE plpgsql STABLE SECURITY DEFINER
SET search_path=pg_catalog,public,app AS $function$
BEGIN RETURN app.release_runtime_schema_readiness_v52()
...
END;
$function$;
CREATE FUNCTION app.ticket_export_runtime_schema_readiness_v52()
RETURNS boolean LANGUAGE plpgsql STABLE SECURITY DEFINER
SET search_path=pg_catalog,public,app AS $function$
BEGIN RETURN app.release_runtime_schema_readiness_v52()
...
END;
$function$;
CREATE FUNCTION app.ticket_metadata_runtime_schema_readiness_v52()
RETURNS boolean LANGUAGE plpgsql STABLE SECURITY DEFINER
SET search_path=pg_catalog,public,app AS $function$
BEGIN RETURN app.release_runtime_schema_readiness_v52()
...
CREATE FUNCTION app.notification_dispatch_readiness_v52()
RETURNS TABLE(
  queue_depth bigint,
  role_safe boolean,
  schema_safe boolean,
...
BEGIN
  ...
  schema_safe := app.release_runtime_schema_readiness_v52();
  RETURN NEXT;
EXCEPTION WHEN undefined_table OR undefined_function OR insufficient_privilege
  OR invalid_schema_name OR cardinality_violation OR data_exception THEN
  queue_depth := 0;
...
ALTER FUNCTION app.federated_authentication_schema_readiness_v52()
  OWNER TO periapsis_migrator;
ALTER FUNCTION app.platform_oidc_direct_runtime_schema_readiness_v52()
  OWNER TO periapsis_migrator;
ALTER FUNCTION app.platform_saml_direct_runtime_schema_readiness_v52()
  OWNER TO periapsis_migrator;
ALTER FUNCTION app.platform_local_account_runtime_schema_readiness_v52()
  OWNER TO periapsis_migrator;
ALTER FUNCTION app.sla_trigger_action_runtime_schema_readiness_v52()
  OWNER TO periapsis_migrator;
ALTER FUNCTION app.sla_object_event_ingress_schema_readiness_v52()
  OWNER TO periapsis_migrator;
ALTER FUNCTION app.ticket_bulk_runtime_schema_readiness_v52()
  OWNER TO periapsis_migrator;
ALTER FUNCTION app.ticket_export_runtime_schema_readiness_v52()
  OWNER TO periapsis_migrator;
ALTER FUNCTION app.ticket_metadata_runtime_schema_readiness_v52()
  OWNER TO periapsis_migrator;
ALTER FUNCTION app.notification_dispatch_readiness_v52()
...
CREATE FUNCTION app.private_rotate_notification_readiness_v52()
...
END;
$function$;
ALTER FUNCTION app.private_rotate_notification_readiness_v52()
  OWNER TO periapsis_notification_readiness_owner;
REVOKE ALL ON FUNCTION app.private_rotate_notification_readiness_v52()
  FROM PUBLIC,periapsis_api,periapsis_worker,periapsis_notifier,
       periapsis_auditor,periapsis_notification_admin_owner,
       periapsis_notification_dispatch_owner;
GRANT EXECUTE ON FUNCTION app.private_rotate_notification_readiness_v52()
...
DECLARE
  ...
  was_sealed boolean;
BEGIN
  IF p_expected_count IS DISTINCT FROM 236
     OR p_expected_latest_created_at IS DISTINCT FROM 1788707601308
     OR p_expected_latest_hash !~ '^[0-9a-f]{64}$'
     OR p_expected_migration_fingerprint !~
       '^[0-9]+@[0-9a-f]{64}(:[0-9]+@[0-9a-f]{64}){235}$' THEN
    RAISE EXCEPTION 'schema compatibility v52 seal input is invalid'
      USING ERRCODE='55000';
  END IF;
  ...
  IF ROW(journal_count,journal_latest_created_at,journal_latest_hash,
     ...
     OR NOT app.private_release_runtime_schema_readiness_v52() THEN
    RAISE EXCEPTION 'schema compatibility v52 pre-seal verification failed'
      USING ERRCODE='55000';
  END IF;

  SELECT coalesce((
    SELECT compatibility.applied_count=236
    FROM app.schema_compatibility_v52() AS compatibility
  ),false) INTO was_sealed;

  IF NOT was_sealed THEN
    EXECUTE pg_catalog.format(
      'ALTER FUNCTION app.schema_compatibility_v52() SET app.schema_compatibility_fingerprint = %L',
      p_expected_migration_fingerprint
    );
    ...
    PERFORM app.private_rotate_notification_readiness_v52();
  ...
  IF NOT was_sealed THEN
    IF sealed_count<>236
       OR NOT app.private_release_runtime_schema_readiness_v52() THEN
      RAISE EXCEPTION 'schema compatibility v52 rotation verification failed'
        USING ERRCODE='55000';
    END IF;
    RETURN;
  END IF;
  IF sealed_count<>236
     OR NOT app.private_release_runtime_schema_readiness_v52()
     OR NOT app.release_runtime_schema_readiness_v52()
     OR NOT app.federated_authentication_schema_readiness_v52()
     OR NOT app.platform_oidc_direct_runtime_schema_readiness_v52()
     OR NOT app.platform_saml_direct_runtime_schema_readiness_v52()
     OR NOT app.platform_local_account_runtime_schema_readiness_v52()
     OR NOT app.sla_trigger_action_runtime_schema_readiness_v52()
     OR NOT app.sla_object_event_ingress_schema_readiness_v52()
     OR NOT app.ticket_bulk_runtime_schema_readiness_v52()
     OR NOT app.ticket_export_runtime_schema_readiness_v52()
     OR NOT app.ticket_metadata_runtime_schema_readiness_v52()
     OR NOT (SELECT readiness.schema_safe
             FROM app.notification_dispatch_readiness_v52() AS readiness) THEN
    RAISE EXCEPTION 'schema compatibility v52 post-seal verification failed'
      USING ERRCODE='55000';
  END IF;
END;
$function$;
