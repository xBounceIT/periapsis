...
function useAlertDfirPanelState({
  alertId,
  api,
  csrfToken,
  hasPermission,
  ...
  useEffect(() => {
    ...
    void queryClient.cancelQueries(filters);
    queryClient.removeQueries(filters);
  }, [canReadWorkspace, queryClient, tenantId, alertId]);
  useLayoutEffect(() => {
...
export function AlertDfirPanelScope(
  props: AlertDfirPanelProps,
): React.JSX.Element {
  const state = useAlertDfirPanelState(props);
  const {
    alertId,
    api,
    csrfToken,
  ...
  const {
    ...
    submitRetraction,
    prepareDownload,
  } = createAlertDfirActions({ ...state, snapshot });
...
function createAlertDfirActions({
  alertId,
  api,
  csrfToken,
  hasPermission,
  ...
  canCreatePanel,
  snapshot,
}: ReturnType<typeof useAlertDfirPanelState> & {
  snapshot: AlertDfirWorkspaceSnapshot;
}) {
...
interface DialogBodyProps {
  busy: ReturnType<typeof useAlertDfirPanelState>["busy"];
  dialog: WorkspaceDialog;
  download?: ReturnType<typeof useAlertDfirPanelState>["download"];
  hasPermission: ReturnType<typeof useAlertDfirPanelState>["hasPermission"];
  onCancel: () => void;
...
interface AlertTaskActionFieldsProps {
  busy: ReturnType<typeof useAlertDfirPanelState>["busy"];
...
function AlertTaskActionFields({
  busy,
  formError,
  kind,
  onCancel,
  ...
  target,
  task,
}: AlertTaskActionFieldsProps): React.JSX.Element {
  return (
    <fieldset disabled={busy}>
      ...
      {formError ? <p role="alert">{formError}</p> : null}
      <FormActions busy={busy} label="Apply task action" onCancel={onCancel} />
    </fieldset>
  );
}
