...
type PolicyRuleEditor = WebhookUrlPolicyRuleDraft & { id: string };

function policyRuleEditors(
  policy: WebhookUrlPolicy | null,
): PolicyRuleEditor[] {
  return webhookUrlPolicyDraft(policy).map((rule) => ({
    ...rule,
...
export function WebhookUrlPolicyPanel(
  props: WebhookUrlPolicyPanelProps,
): React.JSX.Element {
  const model = useWebhookUrlPolicyPanelModel(props);
  return <WebhookUrlPolicyPanelView model={model.data} />;
}

function useWebhookUrlPolicyPanelModel({
  api,
  csrfToken,
  tenantId,
}: WebhookUrlPolicyPanelProps) {
  const [current, setCurrent] = useState<Versioned<WebhookUrlPolicy> | null>(
    null,
  );
  const [rules, setRules] = useState<PolicyRuleEditor[]>(() =>
    policyRuleEditors(null),
  );
  ...
  const load = useCallback(
    async (signal?: AbortSignal): Promise<void> => {
      ...
      try {
        ...
        setCurrent(loaded);
        setRules(policyRuleEditors(loaded?.value ?? null));
        setNotice(null);
        setReady(true);
  ...
  function updateRule(
    ...
    setRules((existing) =>
      existing.map((rule, ruleIndex) =>
        ruleIndex === index ? { ...update(rule), id: rule.id } : rule,
      ),
    );
  }
  ...
  async function publish(event: FormEvent<HTMLFormElement>): Promise<void> {
    ...
    try {
      ...
      setCurrent(visible);
      setRules(policyRuleEditors(visible.value));
      setReady(true);
      setAuditReason("");
      setNotice(
        saved.replayed
...
function WebhookUrlPolicyPanelView({
  model,
}: {
  model: Extract<
    ReturnType<typeof useWebhookUrlPolicyPanelModel>,
    { kind: "ready" }
  >["data"];
}): React.JSX.Element {
  const {
  ...
  return (
    <div className="notification-policy-layout">
      <PolicySummary
        ...
        setPreviewEndpoint={setPreviewEndpoint}
      />
...
interface PolicyRuleFormProps {
  ...
  auditReason: string;
  busy: boolean;
  current: Versioned<WebhookUrlPolicy> | null;
  error: unknown;
  loading: boolean;
  notice: string | null;
  publish: (event: FormEvent<HTMLFormElement>) => Promise<void>;
  ready: boolean;
  rules: PolicyRuleEditor[];
  setAuditReason: React.Dispatch<React.SetStateAction<string>>;
  setRules: React.Dispatch<React.SetStateAction<PolicyRuleEditor[]>>;
  updateRule: (
    index: number,
    update: (current: WebhookUrlPolicyRuleDraft) => WebhookUrlPolicyRuleDraft,
  ) => void;
...
function PolicyRuleForm({
  ...
  setRules,
  updateRule,
}: PolicyRuleFormProps): React.JSX.Element {
  return (
    <section
      ...
      <form onSubmit={(event) => void publish(event)}>
        ...
        <div className="notification-policy-rules">
          {rules.map((rule, index) => (
            <fieldset key={rule.id} className="notification-policy-rule">
              ...
              <FormField htmlFor={`webhook-policy-port-${index}`} label="Port">
                <Input
...
interface PolicySummaryProps {
  ...
  loading: boolean;
  preview: WebhookUrlPolicyPreview;
  previewEndpoint: string;
  setPreviewEndpoint: React.Dispatch<React.SetStateAction<string>>;
}
...
function PolicySummary({
  ...
  previewEndpoint,
  setPreviewEndpoint,
}: PolicySummaryProps): React.JSX.Element {
  return (
    <section className="notification-provider-summary" aria-busy={loading}>
      ...
      <p className="notification-secret-note">
        ...
        configuration pin. Append a webhook configuration version under the new
        policy before delivery resumes.
      </p>
      <WebhookEndpointPreview
        preview={preview}
        previewEndpoint={previewEndpoint}
        setPreviewEndpoint={setPreviewEndpoint}
      />
...
interface WebhookEndpointPreviewProps {
  preview: WebhookUrlPolicyPreview;
  previewEndpoint: string;
  setPreviewEndpoint: React.Dispatch<React.SetStateAction<string>>;
}

function WebhookEndpointPreview({
  preview,
  previewEndpoint,
  setPreviewEndpoint,
}: WebhookEndpointPreviewProps): React.JSX.Element {
  return (
