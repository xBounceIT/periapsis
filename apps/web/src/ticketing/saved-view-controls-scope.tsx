...
function useSavedViewControlsState({
  api,
  canPersist,
  csrfToken,
  currentSpec,
...
function createSavedViewControlsActions({
  api,
  currentSpec,
  onForbidden,
  onMutation,
  ...
  csrfToken,
  tenantId,
}: ReturnType<typeof useSavedViewControlsState>) {
...
export function SavedViewControlsScope(
  props: SavedViewControlsProps,
): React.JSX.Element {
  const state = useSavedViewControlsState(props);
  const {
    canPersist,
    inventoryError,
    inventoryHasMore,
  ...
  const { openNameDialog, submitName, updateSelected, changeLifecycle } =
    createSavedViewControlsActions(state);
  return (
    <section className="saved-view-bar" aria-labelledby="saved-view-title">
      ...
      <SavedViewActions
        ...
        inventoryLoadingMore={inventoryLoadingMore}
...
interface SavedViewActionsProps {
  integrityReady: ReturnType<
    typeof useSavedViewControlsState
  >["integrityReady"];
  inventoryHasMore: ReturnType<
    typeof useSavedViewControlsState
  >["inventoryHasMore"];
  inventoryLoadingMore: ReturnType<
    typeof useSavedViewControlsState
  >["inventoryLoadingMore"];
  onLoadMore: ReturnType<typeof useSavedViewControlsState>["onLoadMore"];
  onSelect: ReturnType<typeof useSavedViewControlsState>["onSelect"];
  openNameDialog: ReturnType<
    typeof createSavedViewControlsActions
  >["openNameDialog"];
  selected: ReturnType<typeof useSavedViewControlsState>["selected"];
  selection: ReturnType<typeof useSavedViewControlsState>["selection"];
  setLifecycleDialog: ReturnType<
    typeof useSavedViewControlsState
  >["setLifecycleDialog"];
  updateSelected: ReturnType<
    typeof createSavedViewControlsActions
  >["updateSelected"];
}
...
function SavedViewActions({
  ...
  setLifecycleDialog,
  updateSelected,
}: SavedViewActionsProps): React.JSX.Element {
  return (
...
interface SavedViewErrorMessageProps {
  conflict: boolean;
  error: unknown;
  onReload: () => void;
}

function SavedViewErrorMessage({
  conflict,
  error,
  onReload,
}: SavedViewErrorMessageProps): React.JSX.Element {
  return (
