...
function useTicketExportControlsState({
  ...
  const [downloadReceipt, setDownloadReceipt] = useState<{
    ...
    value: TicketExportPreparedDownloadView;
  }>();
  const sourceKey = useMemo(() => JSON.stringify(source), [source]);
  const reviewedRequest =
    reviewState?.sourceKey === sourceKey ? reviewState.request : undefined;
  const setReviewedRequest = useCallback(
    (next: SetStateAction<TicketExportJobRequest | undefined>): void => {
      setReviewState((previous) => {
        const current =
          previous?.sourceKey === sourceKey ? previous.request : undefined;
        const request = typeof next === "function" ? next(current) : next;
        return request === undefined ? undefined : { sourceKey, request };
      });
    },
    [sourceKey],
  );
  ...
  }, [allowPrivateComments, allowPublicComments, setReviewedRequest]);
  ...
  if (
    downloadReceipt &&
    downloadReceipt.artifactIdentity !== artifactIdentity
  ) {
    setDownloadReceipt(undefined);
  }
  const preparedDownload =
    downloadReceipt?.artifactIdentity === artifactIdentity
      ? downloadReceipt.value
      : undefined;
  const setPreparedDownload = useCallback(
    (value: TicketExportPreparedDownloadView | undefined): void => {
      setDownloadReceipt(
        value === undefined ? undefined : { artifactIdentity, value },
      );
    },
    [artifactIdentity],
  );
  ...
  }, [preparedDownload, setPreparedDownload]);
  useEffect(() => {
    if (
...
function createTicketExportControlsActions({
  csrfToken,
  kind,
  onForbidden,
  onUnauthorized,
  ...
  retentionSeconds,
  source,
}: ReturnType<typeof useTicketExportControlsState>) {
  ...
  return { review, submit, cancel, prepareDownload, publishJob };
}

export function TicketExportControlsScope(
  props: TicketExportControlsProps,
): React.JSX.Element {
  const state = useTicketExportControlsState(props);
  const {
    allowPrivateComments,
    allowPublicComments,
    csrfToken,
  ...
  const { review, submit, cancel, prepareDownload } =
    createTicketExportControlsActions(state);
  return (
    <section
...
interface TicketExportRequestFormProps {
  allowPrivateComments: ReturnType<
    typeof useTicketExportControlsState
  >["allowPrivateComments"];
  allowPublicComments: ReturnType<
    typeof useTicketExportControlsState
  >["allowPublicComments"];
  comments: ReturnType<typeof useTicketExportControlsState>["comments"];
  csrfToken: ReturnType<typeof useTicketExportControlsState>["csrfToken"];
  maximumRows: ReturnType<typeof useTicketExportControlsState>["maximumRows"];
  pending: ReturnType<typeof useTicketExportControlsState>["pending"];
  retentionSeconds: ReturnType<
    typeof useTicketExportControlsState
  >["retentionSeconds"];
  review: ReturnType<typeof createTicketExportControlsActions>["review"];
  setComments: ReturnType<typeof useTicketExportControlsState>["setComments"];
  setMaximumRows: ReturnType<
    typeof useTicketExportControlsState
  >["setMaximumRows"];
...
function TicketExportRequestForm({
  allowPrivateComments,
  allowPublicComments,
  comments,
  csrfToken,
  ...
  setMaximumRows,
  setRetentionSeconds,
}: TicketExportRequestFormProps): React.JSX.Element {
  return (
