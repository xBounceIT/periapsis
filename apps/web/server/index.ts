...
export function createWebServer(options: WebServerOptions = {}) {
  ...
  const trustedProxies =
    options.trustedProxyCIDRs === undefined
      ...
      : parseTrustedProxyCIDRs(options.trustedProxyCIDRs);
  const server = createServer(
    ...
    (request, response) => {
      ...
      void route(request, response, apiBaseUrl, trustedProxies).catch(
        (error: unknown) => {
...
export function parseTrustedProxyCIDRs(raw: string): TrustedProxySet {
