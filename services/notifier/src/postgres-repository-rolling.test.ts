...
describe("PostgreSQL notification rolling compatibility", () => {
  ...
  it("binds current V60 readiness to all five generated source pins and the exact journal", async () => {
    const repository = createRepository();
    database.outcomes.push(
      { rows: [{ available: true }] },
      {
    ...
    expect(database.statements[1]).toContain(
      "FROM app.notification_dispatch_readiness_v60()",
    );
  ...
  it("does not downgrade V60 readiness on authorization failures", async () => {
    const repository = createRepository();
  ...
  it("does not downgrade when a present V60 root raises undefined_function", async () => {
    const repository = createRepository();
  ...
  it("fails closed without a V4 downgrade when the V60 source chain drifts", async () => {
    const repository = createRepository();
    database.outcomes.push({
      rows: [{ available: true }],
    });
