...
with expected_trusted_function(
  ...
  values
    ...
    (9, 'sla_object_event_ingress',
     ...
     'plpgsql', array['search_path=pg_catalog, public, app']::text[],
     'boolean', ` + trustedWorkerACL + `),
    (10, 'ticket_bulk', 'app.ticket_bulk_runtime_schema_readiness_v56()',
     'plpgsql', array['search_path=pg_catalog, public, app']::text[],
     'boolean', ` + trustedRuntimeACL + `),
    (11, 'ticket_export', 'app.ticket_export_runtime_schema_readiness_v56()',
     'plpgsql', array['search_path=pg_catalog, public, app']::text[],
     'boolean', ` + trustedRuntimeACL + `),
    ...
    (14, 'retired_v51', 'app.schema_compatibility_v51()', 'plpgsql', array[
     ...
     'TABLE(applied_count bigint, latest_created_at bigint, latest_hash text, migration_fingerprint text)',
     ` + trustedOwnerACL + `),
    (15, 'worker_aggregate', 'app.worker_runtime_schema_readiness_v56()',
     'plpgsql', array['search_path=pg_catalog, public, app']::text[],
     'boolean[]', ` + trustedWorkerACL + `),
...
trusted_function_state as (
  select array_agg(source_hash order by ordinal) as source_hashes,
         count(*) = 19 and coalesce(bool_and(catalog_ready), false)
           as catalog_ready
...
select applied_count, latest_created_at, latest_hash, migration_fingerprint,
       ...
       app.worker_runtime_schema_readiness_v56() AS array,
...
var expectedTrustedFunctionSourceHashes = [...]string{
	...
	expectedSLATriggerActionRuntimeReadinessV56SourceHash,
	expectedSLAObjectEventIngressReadinessV56SourceHash,
	...
	expectedWorkerRuntimeReadinessV56SourceHash,
