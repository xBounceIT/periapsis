...
func TestSchemaCompatibilityQuerySealsV60TrustedSet(t *testing.T) {
	if len(expectedTrustedFunctionSourceHashes) != 23 {
		t.Fatalf("trusted source hash count = %d, want 23", len(expectedTrustedFunctionSourceHashes))
	}
	...
	if expectedTrustedFunctionSourceHashes[14] != expectedWorkerRuntimeReadinessV60SourceHash {
	...
	roots := []struct {
		...
		{"app.worker_runtime_schema_readiness_v60()", 2},
		...
		{"app.sla_trigger_action_runtime_schema_readiness_v60()", 1},
		{"app.sla_object_event_ingress_schema_readiness_v60()", 1},
