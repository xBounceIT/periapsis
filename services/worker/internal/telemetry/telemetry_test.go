...
func TestWorkerOperationAllowlistIncludesOIDCMaintenanceAndRejectsUnboundedNames(t *testing.T) {
	for _, operation := range []string{
		"auth.cleanup", "database.readiness", "ldap.sync.poll", "sla.action", "sla.evaluate",
		"sla.event_ingress", "oidc.maintenance.expire_access_lease", "oidc.maintenance.cleanup_retention",
		"oidc.maintenance.claim.logout_retry", "oidc.maintenance.claim.refresh",
		...
		if canonicalWorkerOperation(operation) != operation {
			t.Fatalf("allowlisted worker operation %q was disabled", operation)
		}
	}
	for _, operation := range []string{"", "sla.evaluate.tenant", "SLA.evaluate", "customer@example.invalid"} {
		if canonicalWorkerOperation(operation) != "" {
			t.Fatalf("unbounded worker operation %q was accepted", operation)
		}
	}

	runtime, exporter := inMemoryRuntime(t)
	ctx, finish := runtime.StartOperation(context.Background(), "oidc.maintenance.logout_retry")
	if span := trace.SpanContextFromContext(ctx); !span.IsValid() || !span.IsSampled() {
		t.Fatal("OIDC maintenance operation did not create an active trace context")
	}
	finish(errors.New("tenant@example.invalid secret"))
	spans := exporter.GetSpans()
	if len(spans) != 1 || spans[0].Name != "worker.oidc.maintenance.logout_retry" ||
		spans[0].Status.Code != codes.Error ||
		strings.Contains(fmt.Sprint(spans[0]), "tenant@example.invalid") ||
		strings.Contains(fmt.Sprint(spans[0]), "secret") {
		t.Fatalf("OIDC maintenance span was absent, successful, or exposed error material: %#v", spans)
	}
}
