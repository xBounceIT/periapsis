...
func TestHealthCheckerRequiresExactSchemaCompatibility(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "exact v60 state", wantReady: true},
...
func TestHealthCheckerRequiresEveryV60RuntimeAndTrustedRoot(t *testing.T) {
	for index := range 8 {
...
func TestSchemaCompatibilityQuerySealsV60TrustedSet(t *testing.T) {
	if len(expectedTrustedFunctionSourceHashes) != 26 {
		t.Fatalf("trusted source hash count = %d, want 26", len(expectedTrustedFunctionSourceHashes))
	}
	...
	if expectedTrustedFunctionSourceHashes[17] != expectedAPIRuntimeReadinessV60SourceHash {
	...
	for _, required := range []string{
		...
		"(18, 'api_aggregate', 'app.api_runtime_schema_readiness_v60()'",
		"app.api_runtime_schema_readiness_v60() AS array",
		...
		"'compatibility', 'retired', 'retired_v50', 'retired_v51', 'retired_v52', 'retired_v53', 'retired_v54', 'retired_v55', 'retired_v56', 'retired_v57', 'retired_v58', 'retired_v59', 'journal'",
		...
		"count(*) = 26 and coalesce(bool_and(catalog_ready), false)",
		trustedAPIACL,
	...
	if got := strings.Count(schemaCompatibilityQuery, trustedOwnerACL); got != 15 {
		t.Fatalf("owner-only trusted-root ACL count = %d, want 15", got)
	}
	...
	roots := []struct {
		...
		{"app.schema_compatibility_v60()", 2},
		{"app.api_runtime_schema_readiness_v60()", 2},
		{"app.private_v47_migration_convergence_schema_readiness_v1()", 1},
		{"app.private_schema_compatibility_journal_v60()", 1},
		{"app.private_release_runtime_dependency_surface_hash_v60()", 1},
		{"app.private_release_runtime_schema_readiness_v60()", 1},
		{"app.release_runtime_schema_readiness_v60()", 1},
		{"app.federated_authentication_schema_readiness_v60()", 1},
		{"app.platform_oidc_direct_runtime_schema_readiness_v60()", 1},
		{"app.platform_saml_direct_runtime_schema_readiness_v60()", 1},
		{"app.platform_local_account_runtime_schema_readiness_v60()", 1},
		{"app.ticket_bulk_runtime_schema_readiness_v60()", 1},
		{"app.ticket_export_runtime_schema_readiness_v60()", 1},
		{"app.ticket_metadata_runtime_schema_readiness_v60()", 1},
