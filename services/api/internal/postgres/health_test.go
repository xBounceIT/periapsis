...
func TestHealthCheckerRequiresExactSchemaCompatibility(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "exact v56 state", wantReady: true},
...
func TestHealthCheckerRequiresEveryV56RuntimeAndTrustedRoot(t *testing.T) {
	for index := range 8 {
...
func TestSchemaCompatibilityQuerySealsV56TrustedSet(t *testing.T) {
	if len(expectedTrustedFunctionSourceHashes) != 22 {
		t.Fatalf("trusted source hash count = %d, want 22", len(expectedTrustedFunctionSourceHashes))
	}
	...
	if expectedTrustedFunctionSourceHashes[17] != expectedAPIRuntimeReadinessV56SourceHash {
	...
	if got := strings.Count(schemaCompatibilityQuery, trustedOwnerACL); got != 11 {
		t.Fatalf("owner-only trusted-root ACL count = %d, want 11", got)
	}
	...
	roots := []struct {
		...
		{"app.schema_compatibility_v56()", 2},
		{"app.api_runtime_schema_readiness_v56()", 2},
		{"app.private_v47_migration_convergence_schema_readiness_v1()", 1},
		{"app.private_schema_compatibility_journal_v56()", 1},
		{"app.private_release_runtime_dependency_surface_hash_v56()", 1},
		{"app.private_release_runtime_schema_readiness_v56()", 1},
		{"app.release_runtime_schema_readiness_v56()", 1},
		{"app.federated_authentication_schema_readiness_v56()", 1},
		{"app.platform_oidc_direct_runtime_schema_readiness_v56()", 1},
		{"app.platform_saml_direct_runtime_schema_readiness_v56()", 1},
		{"app.platform_local_account_runtime_schema_readiness_v56()", 1},
		{"app.ticket_bulk_runtime_schema_readiness_v56()", 1},
		{"app.ticket_export_runtime_schema_readiness_v56()", 1},
		{"app.ticket_metadata_runtime_schema_readiness_v56()", 1},
