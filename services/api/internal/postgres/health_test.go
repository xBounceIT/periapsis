...
func TestHealthCheckerRequiresExactSchemaCompatibility(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "exact v57 state", wantReady: true},
...
func TestHealthCheckerRequiresEveryV57RuntimeAndTrustedRoot(t *testing.T) {
	for index := range 8 {
...
func TestSchemaCompatibilityQuerySealsV57TrustedSet(t *testing.T) {
	...
	if expectedTrustedFunctionSourceHashes[17] != expectedAPIRuntimeReadinessV57SourceHash {
	...
	if got := strings.Count(schemaCompatibilityQuery, trustedOwnerACL); got != 12 {
		t.Fatalf("owner-only trusted-root ACL count = %d, want 12", got)
	}
	...
	roots := []struct {
		...
		{"app.schema_compatibility_v57()", 2},
		{"app.api_runtime_schema_readiness_v57()", 2},
		{"app.private_v47_migration_convergence_schema_readiness_v1()", 1},
		{"app.private_schema_compatibility_journal_v57()", 1},
		{"app.private_release_runtime_dependency_surface_hash_v57()", 1},
		{"app.private_release_runtime_schema_readiness_v57()", 1},
		{"app.release_runtime_schema_readiness_v57()", 1},
		{"app.federated_authentication_schema_readiness_v57()", 1},
		{"app.platform_oidc_direct_runtime_schema_readiness_v57()", 1},
		{"app.platform_saml_direct_runtime_schema_readiness_v57()", 1},
		{"app.platform_local_account_runtime_schema_readiness_v57()", 1},
		{"app.ticket_bulk_runtime_schema_readiness_v57()", 1},
		{"app.ticket_export_runtime_schema_readiness_v57()", 1},
		{"app.ticket_metadata_runtime_schema_readiness_v57()", 1},
