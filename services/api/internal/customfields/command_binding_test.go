...
func TestCommandBindingPreservesExactDigestBytes(t *testing.T) {
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			for _, operation := range []string{operationDefinitionCreate, operationDefinitionCreate + ".other", ""} {
				got, err := bindCommand(operation, "fixture-idempotency-key", tc.payload)
				if err != nil {
					t.Fatal(err)
				}
				...
				repeated, err := bindCommand(operation, "different-fixture-key", tc.payload)
...
func TestCommandBindingRejectsUnencodablePayload(t *testing.T) {
	for _, payload := range []any{math.NaN(), make(chan int), func() {}} {
		got, err := bindCommand(operationDefinitionCreate, "fixture-idempotency-key", payload)
